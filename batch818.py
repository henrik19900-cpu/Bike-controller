...
FOX_COLOR = "#6ee7b7"; FOX_GLOW = "#ecfdf5"; FOX_PEAK = "💚"
ORB_COLOR = "#292524"; ORB_GLOW = "#fafaf9"; ORB_PEAK = "🔩"
FOX_SC   = 620;  ORB_SC  = 615
FOX_R    = "BASE_R*2.63"; ORB_R = "BASE_R*2.62"
OSC      = 0.4521; SW = 2430
PW1 = "VEIL_GLOW44";  PW1_SC = 2736; PW1_SHOCK = "#7e22ce"; PW1_ICON = "🌫️💫"
PW2 = "DUSK2_GLOW44"; PW2_SC = 2738; PW2_SHOCK = "#4f46e5"; PW2_ICON = "🌇✨"

# Prev-batch anchors (batch 817)
PREV_FOX_FULL = "uraninite_fox9e"
PREV_FOX_COLOR = "#1c1917"; PREV_FOX_GLOW = "#e7e5e4"
PREV_PW1  = "CLAY_GLOW44"; PREV_PW1_ICON = "🏺💫"
PREV_DRAW_FOX = "drawUraniniteF9e"
PREV_FOX_R = "BASE_R*2.62"

# ── Step 1 – 8 achievements ───────────────────────────────────────────────────
ACH = f'''  {{ id:"{FOX_TYPE}_tap",          label:"Wardite Fox Tap",             desc:"Tap Wardite Fox target",                icon:"{FOX_PEAK}", xp:62 }},
  {{ id:"{FOX_TYPE}_peak",         label:"Wardite Fox Peak",            desc:"Reach peak with Wardite Fox",           icon:"{FOX_PEAK}", xp:124 }},
  {{ id:"{ORB_TYPE}_tap",          label:"Wolframite Orb Tap",          desc:"Tap Wolframite Orb target",             icon:"{ORB_PEAK}", xp:62 }},
  {{ id:"{ORB_TYPE}_peak",         label:"Wolframite Orb Peak",         desc:"Reach peak with Wolframite Orb",        icon:"{ORB_PEAK}", xp:124 }},
  {{ id:"{PW1.lower()}_use",       label:"Veil Glow",                   desc:"Trigger VEIL_GLOW44 power-up",          icon:"🌫️", xp:62 }},
  {{ id:"{PW1.lower()}_max",       label:"Veil Glow Max",               desc:"Trigger VEIL_GLOW44 at max streak",     icon:"🌫️", xp:124 }},
  {{ id:"{PW2.lower()}_use",       label:"Dusk2 Glow",                  desc:"Trigger DUSK2_GLOW44 power-up",         icon:"🌇", xp:62 }},
  {{ id:"{PW2.lower()}_max",       label:"Dusk2 Glow Max",              desc:"Trigger DUSK2_GLOW44 at max streak",    icon:"🌇", xp:124 }},
...
CMT = f'''// {PW1} — +{PW1_SC} veil glow bonus
    // {PW2} — +{PW2_SC} dusk2 glow bonus
    // {PREV_PW1}'''
...
DRAWS = f'''function drawWarditeFox9e(ctx,r,ts,sp){{
  const p=ts/sp,pu=p<0.5?p*2:2-p*2;
  ctx.save();
  ctx.shadowColor="{FOX_GLOW}";ctx.shadowBlur=20+pu*18;
  ctx.beginPath();ctx.arc(0,0,r,0,Math.PI*2);
  const g=ctx.createRadialGradient(0,0,r*0.1,0,0,r);
  g.addColorStop(0,"#d1fae5");g.addColorStop(0.5,"{FOX_COLOR}");g.addColorStop(1,"#065f46");
...
function drawWolframiteOrb9e(ctx,r,ts,tp){{
  const p=ts/tp,pu=p<0.5?p*2:2-p*2;
  ctx.save();
  ctx.shadowColor="{ORB_GLOW}";ctx.shadowBlur=18+pu*16;
  ctx.beginPath();ctx.arc(0,0,r,0,Math.PI*2);
  const g=ctx.createRadialGradient(0,0,r*0.1,0,0,r);
  g.addColorStop(0,"#78716c");g.addColorStop(0.5,"{ORB_COLOR}");g.addColorStop(1,"#000000");
  ctx.fillStyle=g;ctx.fill();
...
DISP = f'''  else if(t.type==="{FOX_TYPE}"){{drawWarditeFox9e(ctx,t.radius,ts-t.born,t.lifetime);}}
  else if(t.type==="{ORB_TYPE}"){{drawWolframiteOrb9e(ctx,t.radius,ts-t.born,t.lifetime);}}
...
with open(SRC, "w") as f:
    f.write(src)
print(f"Batch 818 done! +{len(src)-original_len} bytes")
