...
OSC   = "0.4277"
SW    = "2308"
FOX_C = "#9a3412"; FOX_G = "#fff7ed"
ORB_C = "#134e4a"; ORB_G = "#f0fdfa"
FOX_L = "#fed7aa"; FOX_M = "#ea580c"; FOX_D = "#431407"
FOX_RL,FOX_RG,FOX_RB = 254,215,170
ORB_L = "#99f6e4"; ORB_M = "#0d9488"; ORB_D = "#042f2e"
ORB_RL,ORB_RG,ORB_RB = 153,246,228
PW1C = "#a5f3fc"; PW2C = "#bae6fd"
PEAK_FOX = "🧡"; PEAK_ORB = "🌿"

# STEP 1
A1_ANCHOR = '{ id:"elbaite_fox9e_tap", label:"Elbaite Fox Tap"'
assert A1_ANCHOR in code, "S1 anchor missing"
A1_NEW = (
  '{ id:"enargite_fox9e_tap", label:"Enargite Fox Tap", desc:"Tap Enargite Fox", icon:"🦊", xp:62 },\n'
  '  { id:"enargite_fox9e_peak", label:"Enargite Fox Peak", desc:"Hit Enargite Fox at peak", icon:"🧡", xp:78 },\n'
  '  { id:"eosphorite_orb9e_tap", label:"Eosphorite Orb Tap", desc:"Tap Eosphorite Orb", icon:"🔮", xp:60 },\n'
  '  { id:"eosphorite_orb9e_peak", label:"Eosphorite Orb Peak", desc:"Hit Eosphorite Orb at peak", icon:"🌿", xp:76 },\n'
  '  { id:"tundra_glow44_use", label:"Tundra Glow Use", desc:"Collect Tundra Glow power-up", icon:"🌨️", xp:44 },\n'
  '  { id:"tundra_glow44_max", label:"Tundra Glow Max", desc:"Collect 5 Tundra Glow power-ups", icon:"🌨️", xp:88 },\n'
  '  { id:"polar_glow44_use", label:"Polar Glow Use", desc:"Collect Polar Glow power-up", icon:"🧊", xp:44 },\n'
  '  { id:"polar_glow44_max", label:"Polar Glow Max", desc:"Collect 5 Polar Glow power-ups", icon:"🧊", xp:88 },\n'
  '  { id:"elbaite_fox9e_tap", label:"Elbaite Fox Tap"'
)
code = code.replace(A1_ANCHOR, A1_NEW, 1)
assert '{ id:"enargite_fox9e_tap"' in code, "S1 failed"
print("S1 ok")

# STEP 2
A2_ANCHOR = '"GLACIER_GLOW44","ARCTIC_GLOW44","BLIZZARD_GLOW44"'
assert A2_ANCHOR in code, "S2 anchor missing"
A2_NEW = f'"{PW1}","{PW2}","GLACIER_GLOW44","ARCTIC_GLOW44","BLIZZARD_GLOW44"'
code = code.replace(A2_ANCHOR, A2_NEW, 1)
assert f'"{PW1}"' in code, "S2 failed"
print("S2 ok")

# STEP 3
A3_ANCHOR = '} else if(ptype==="GLACIER_GLOW44"){'
...
# STEP 4
A4_ANCHOR = '// GLACIER_GLOW44 — +2488 glacier glow bonus'
assert A4_ANCHOR in code, "S4 anchor missing"
A4_NEW = (
  f'// {PW1} — +{PW1SC} tundra glow bonus\n'
  f'      // {PW2} — +{PW2SC} polar glow bonus\n'
  f'      // GLACIER_GLOW44 — +2488 glacier glow bonus'
)
...
# STEP 5
A5_ANCHOR = 'function drawElbaiteFox9e('
assert A5_ANCHOR in code, "S5 anchor missing"
DRAW = (
f'function drawEnargiteFox9e(ctx,r,ts,sp){{\n'
...
f'  ctx.restore();\n'
f'}}\n'
f'function drawEosphoriteOrb9e(ctx,r,ts,tp){{\n'
...
f'  ctx.restore();\n'
f'}}\n'
f'function drawElbaiteFox9e('
)
code = code.replace(A5_ANCHOR, DRAW, 1)
assert 'function drawEnargiteFox9e(' in code, "S5 failed"
print("S5 ok")

# STEP 6
A6_ANCHOR = '  else if(t.type==="elbaite_fox9e"){'
...
f'    drawEnargiteFox9e(ctx,t.radius,ts,sp2);ctx.restore();\n'
f'  }}\n'
f'  else if(t.type==="{ORB}"){{\n'
f'    ctx.save();ctx.translate(t.x,t.y);\n'
f'    const tp2=Math.min(1,(ts-t.born)/{SW});\n'
f'    drawEosphoriteOrb9e(ctx,t.radius,ts,tp2);ctx.restore();\n'
f'  }}\n'
f'  else if(t.type==="elbaite_fox9e"){{'
)
...
# STEP 7
A7_ANCHOR = 'if(hit.type==="elbaite_fox9e"){'
...
f'      if(isPeak)unlock("enargite_fox9e_peak");\n'
f'      unlock("enargite_fox9e_tap");\n'
...
f'      addParticle(hit.x,hit.y,"popup",isPeak?"PEAK! +"+pts:"🔮 +"+pts);\n'
f'      if(isPeak)unlock("eosphorite_orb9e_peak");\n'
f'      unlock("eosphorite_orb9e_tap");\n'
f'      updateMissions({{...gs.sessionStats,rareHits:gs.sessionStats.rareHits}});\n'
f'      sfx("epic");vibrate([40,10,20]);\n'
f'    }}else if(hit.type==="elbaite_fox9e"){{'
)
...
# STEP 8
A8_OLD = ('      type="elbaite_fox9e";color="#0f766e";glow="#f0fdfa";\n'
          '    } else if('+COND100F+'){\n'
          '      type="emplectite_orb9e"')
...
A8_NEW = (
    ...
    f'    }} else if({COND100F}){{\n'
    f'      type="elbaite_fox9e";color="#0f766e";glow="#f0fdfa";\n'
    f'    }} else if({COND100F}){{\n'
    f'      type="emplectite_orb9e"'
)
...
# STEP 9
A9_ANCHOR = 'type==="elbaite_fox9e"?BASE_R*2.01:type==="emplectite_orb9e"?BASE_R*2.00:'
...
# STEP 10
A10_ANCHOR = 'GLACIER_GLOW44:"🧊✨",ARCTIC_GLOW44:"❄️✨",BLIZZARD_GLOW44:"❄️✨"'
cnt = code.count(A10_ANCHOR)
assert cnt == 2, f"S10 anchor count={cnt}, expected 2"
A10_NEW = f'{PW1}:"🌨️✨",{PW2}:"🧊✨",GLACIER_GLOW44:"🧊✨",ARCTIC_GLOW44:"❄️✨",BLIZZARD_GLOW44:"❄️✨"'
...
with open(SRC, "w") as f:
    f.write(code)
print(f"Batch 757 done! +{len(code)-orig_len} bytes")
