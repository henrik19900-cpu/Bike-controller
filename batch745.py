...
FOX   = "chalcanthite_fox9e"
ORB   = "chkalovite_orb9e"
PW1   = "LUSTER_GLOW44"
PW2   = "RADIANCE_GLOW44"
PW1SC = 2444
PW2SC = 2446
...
FOX_L = "#ddd6fe"; FOX_M = "#8b5cf6"; FOX_D = "#2e1065"
FOX_RL,FOX_RG,FOX_RB = 221,214,254
ORB_L = "#fde68a"; ORB_M = "#d97706"; ORB_D = "#451a03"
ORB_RL,ORB_RG,ORB_RB = 253,230,138
PW1C = "#a78bfa"; PW2C = "#fb923c"
PEAK_FOX = "⚡"; PEAK_ORB = "🌟"

# STEP 1
A1_ANCHOR = '{ id:"celestine_fox9e_tap", label:"Celestine Fox Tap"'
assert A1_ANCHOR in code, "S1 anchor missing"
A1_NEW = (
  '{ id:"chalcanthite_fox9e_tap", label:"Chalcanthite Fox Tap", desc:"Tap Chalcanthite Fox", icon:"🦊", xp:62 },\n'
  '  { id:"chalcanthite_fox9e_peak", label:"Chalcanthite Fox Peak", desc:"Hit Chalcanthite Fox at peak", icon:"⚡", xp:78 },\n'
  '  { id:"chkalovite_orb9e_tap", label:"Chkalovite Orb Tap", desc:"Tap Chkalovite Orb", icon:"🔮", xp:60 },\n'
  '  { id:"chkalovite_orb9e_peak", label:"Chkalovite Orb Peak", desc:"Hit Chkalovite Orb at peak", icon:"🌟", xp:76 },\n'
  '  { id:"luster_glow44_use", label:"Luster Glow Use", desc:"Collect Luster Glow power-up", icon:"💜", xp:44 },\n'
  '  { id:"luster_glow44_max", label:"Luster Glow Max", desc:"Collect 5 Luster Glow power-ups", icon:"💜", xp:88 },\n'
  '  { id:"radiance_glow44_use", label:"Radiance Glow Use", desc:"Collect Radiance Glow power-up", icon:"🌅", xp:44 },\n'
  '  { id:"radiance_glow44_max", label:"Radiance Glow Max", desc:"Collect 5 Radiance Glow power-ups", icon:"🌅", xp:88 },\n'
  '  { id:"celestine_fox9e_tap", label:"Celestine Fox Tap"'
)
code = code.replace(A1_ANCHOR, A1_NEW, 1)
assert '{ id:"chalcanthite_fox9e_tap"' in code, "S1 failed"
print("S1 ok")

# STEP 2
A2_ANCHOR = '"GLEAM_GLOW44","GLINT_GLOW44","FLARE_GLOW44"'
assert A2_ANCHOR in code, "S2 anchor missing"
A2_NEW = f'"{PW1}","{PW2}","GLEAM_GLOW44","GLINT_GLOW44","FLARE_GLOW44"'
code = code.replace(A2_ANCHOR, A2_NEW, 1)
assert f'"{PW1}"' in code, "S2 failed"
print("S2 ok")

# STEP 3
A3_ANCHOR = '} else if(ptype==="GLEAM_GLOW44"){'
...
# STEP 4
A4_ANCHOR = '// GLEAM_GLOW44 — +2440 gleam glow bonus'
assert A4_ANCHOR in code, "S4 anchor missing"
A4_NEW = (
  f'// {PW1} — +{PW1SC} luster glow bonus\n'
  f'      // {PW2} — +{PW2SC} radiance glow bonus\n'
  f'      // GLEAM_GLOW44 — +2440 gleam glow bonus'
)
...
# STEP 5
A5_ANCHOR = 'function drawCelestineFox9e('
assert A5_ANCHOR in code, "S5 anchor missing"
DRAW = (
f'function drawChalcanthiteFox9e(ctx,r,ts,sp){{\n'
...
f'  ctx.restore();\n'
f'}}\n'
f'function drawChkaloviteOrb9e(ctx,r,ts,tp){{\n'
...
f'  ctx.restore();\n'
f'}}\n'
f'function drawCelestineFox9e('
)
code = code.replace(A5_ANCHOR, DRAW, 1)
assert 'function drawChalcanthiteFox9e(' in code, "S5 failed"
print("S5 ok")

# STEP 6
A6_ANCHOR = '  else if(t.type==="celestine_fox9e"){'
...
f'    drawChalcanthiteFox9e(ctx,t.radius,ts,sp2);ctx.restore();\n'
f'  }}\n'
f'  else if(t.type==="{ORB}"){{\n'
f'    ctx.save();ctx.translate(t.x,t.y);\n'
f'    const tp2=Math.min(1,(ts-t.born)/{SW});\n'
f'    drawChkaloviteOrb9e(ctx,t.radius,ts,tp2);ctx.restore();\n'
f'  }}\n'
f'  else if(t.type==="celestine_fox9e"){{'
)
...
# STEP 7
A7_ANCHOR = 'if(hit.type==="celestine_fox9e"){'
...
f'      if(isPeak)unlock("chalcanthite_fox9e_peak");\n'
f'      unlock("chalcanthite_fox9e_tap");\n'
...
f'      addParticle(hit.x,hit.y,"popup",isPeak?"PEAK! +"+pts:"🔮 +"+pts);\n'
f'      if(isPeak)unlock("chkalovite_orb9e_peak");\n'
f'      unlock("chkalovite_orb9e_tap");\n'
f'      updateMissions({{...gs.sessionStats,rareHits:gs.sessionStats.rareHits}});\n'
f'      sfx("epic");vibrate([40,10,20]);\n'
f'    }}else if(hit.type==="celestine_fox9e"){{'
)
...
# STEP 8
A8_OLD = ('      type="celestine_fox9e";color="#0369a1";glow="#e0f2fe";\n'
          '    } else if('+COND100F+'){\n'
          '      type="chabazite_orb9e"')
...
A8_NEW = (
    ...
    f'    }} else if({COND100F}){{\n'
    f'      type="celestine_fox9e";color="#0369a1";glow="#e0f2fe";\n'
    f'    }} else if({COND100F}){{\n'
    f'      type="chabazite_orb9e"'
)
...
# STEP 9
A9_ANCHOR = 'type==="celestine_fox9e"?BASE_R*1.89:type==="chabazite_orb9e"?BASE_R*1.88:'
...
# STEP 10
A10_ANCHOR = 'GLEAM_GLOW44:"✨✨",GLINT_GLOW44:"💎✨",FLARE_GLOW44:"🔥✨"'
cnt = code.count(A10_ANCHOR)
assert cnt == 2, f"S10 anchor count={cnt}, expected 2"
A10_NEW = f'{PW1}:"💜✨",{PW2}:"🌅✨",GLEAM_GLOW44:"✨✨",GLINT_GLOW44:"💎✨",FLARE_GLOW44:"🔥✨"'
...
with open(SRC, "w") as f:
    f.write(code)
print(f"Batch 745 done! +{len(code)-orig_len} bytes")
