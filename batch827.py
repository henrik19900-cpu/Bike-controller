...
FOX_COLOR = "#334155"; FOX_GLOW = "#e2e8f0"; FOX_PEAK = "🌚"
ORB_COLOR = "#854d0e"; ORB_GLOW = "#fef3c7"; ORB_PEAK = "🟠"
FOX_SC   = 638;  ORB_SC  = 633
FOX_R    = "BASE_R*2.72"; ORB_R = "BASE_R*2.71"
OSC      = 0.4557; SW = 2448
PW1 = "SPARK2_GLOW44"; PW1_SC = 2772; PW1_SHOCK = "#fbbf24"; PW1_ICON = "✨💫"
PW2 = "EMBER2_GLOW44"; PW2_SC = 2774; PW2_SHOCK = "#f59e0b"; PW2_ICON = "🔥✨"

# Prev-batch anchors (batch 826)
PREV_FOX_FULL = "aramayoite_fox9e"
PREV_FOX_COLOR = "#64748b"; PREV_FOX_GLOW = "#f1f5f9"
PREV_PW1  = "BLAZE2_GLOW44"; PREV_PW1_ICON = "🔥💫"
PREV_DRAW_FOX = "drawAramayoiteFox9e"
PREV_FOX_R = "BASE_R*2.71"

# ── Step 1 – 8 achievements ───────────────────────────────────────────────────
ACH = f'''  {{ id:"{FOX_TYPE}_tap",          label:"Argentopyrite Fox Tap",       desc:"Tap Argentopyrite Fox target",          icon:"{FOX_PEAK}", xp:62 }},
  {{ id:"{FOX_TYPE}_peak",         label:"Argentopyrite Fox Peak",      desc:"Reach peak with Argentopyrite Fox",     icon:"{FOX_PEAK}", xp:124 }},
  {{ id:"{ORB_TYPE}_tap",          label:"Arseniosiderite Orb Tap",     desc:"Tap Arseniosiderite Orb target",        icon:"{ORB_PEAK}", xp:62 }},
  {{ id:"{ORB_TYPE}_peak",         label:"Arseniosiderite Orb Peak",    desc:"Reach peak with Arseniosiderite Orb",   icon:"{ORB_PEAK}", xp:124 }},
  {{ id:"{PW1.lower()}_use",       label:"Spark2 Glow",                 desc:"Trigger SPARK2_GLOW44 power-up",        icon:"✨", xp:62 }},
  {{ id:"{PW1.lower()}_max",       label:"Spark2 Glow Max",             desc:"Trigger SPARK2_GLOW44 at max streak",   icon:"✨", xp:124 }},
  {{ id:"{PW2.lower()}_use",       label:"Ember2 Glow",                 desc:"Trigger EMBER2_GLOW44 power-up",        icon:"🔥", xp:62 }},
  {{ id:"{PW2.lower()}_max",       label:"Ember2 Glow Max",             desc:"Trigger EMBER2_GLOW44 at max streak",   icon:"🔥", xp:124 }},
...
CMT = f'''// {PW1} — +{PW1_SC} spark2 glow bonus
    // {PW2} — +{PW2_SC} ember2 glow bonus
    // {PREV_PW1}'''
...
DRAWS = f'''function drawArgentopyriteFox9e(ctx,r,ts,sp){{
...
function drawArseniosideriteOrb9e(ctx,r,ts,tp){{
...
DISP = f'''  else if(t.type==="{FOX_TYPE}"){{drawArgentopyriteFox9e(ctx,t.radius,ts-t.born,t.lifetime);}}
  else if(t.type==="{ORB_TYPE}"){{drawArseniosideriteOrb9e(ctx,t.radius,ts-t.born,t.lifetime);}}
...
with open(SRC, "w") as f:
    f.write(src)
print(f"Batch 827 done! +{len(src)-original_len} bytes")
