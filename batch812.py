...
FOX_COLOR = "#ea580c"; FOX_GLOW = "#fff7ed"; FOX_PEAK = "🍊"
ORB_COLOR = "#0284c7"; ORB_GLOW = "#e0f2fe"; ORB_PEAK = "🌊"
FOX_SC   = 608;  ORB_SC  = 603
FOX_R    = "BASE_R*2.57"; ORB_R = "BASE_R*2.56"
OSC      = 0.4497; SW = 2418
PW1 = "FERN_GLOW44";  PW1_SC = 2712; PW1_SHOCK = "#4d7c0f"; PW1_ICON = "🌿💫"
PW2 = "TWIG_GLOW44";  PW2_SC = 2714; PW2_SHOCK = "#365314"; PW2_ICON = "🪵✨"

# Prev-batch anchors (batch 811)
PREV_FOX_FULL = "tetradymite_fox9e"
PREV_FOX_COLOR = "#f59e0b"; PREV_FOX_GLOW = "#fffbeb"
PREV_PW1  = "GROVE_GLOW44"; PREV_PW1_ICON = "🌲💫"
PREV_DRAW_FOX = "drawTetradymiteFox9e"
PREV_FOX_R = "BASE_R*2.56"

# ── Step 1 – 8 achievements ───────────────────────────────────────────────────
ACH = f'''  {{ id:"{FOX_TYPE}_tap",          label:"Tinaksite Fox Tap",           desc:"Tap Tinaksite Fox target",              icon:"{FOX_PEAK}", xp:62 }},
  {{ id:"{FOX_TYPE}_peak",         label:"Tinaksite Fox Peak",          desc:"Reach peak with Tinaksite Fox",         icon:"{FOX_PEAK}", xp:124 }},
  {{ id:"{ORB_TYPE}_tap",          label:"Tacharanite Orb Tap",         desc:"Tap Tacharanite Orb target",            icon:"{ORB_PEAK}", xp:62 }},
  {{ id:"{ORB_TYPE}_peak",         label:"Tacharanite Orb Peak",        desc:"Reach peak with Tacharanite Orb",       icon:"{ORB_PEAK}", xp:124 }},
  {{ id:"{PW1.lower()}_use",       label:"Fern Glow",                   desc:"Trigger FERN_GLOW44 power-up",          icon:"🌿", xp:62 }},
  {{ id:"{PW1.lower()}_max",       label:"Fern Glow Max",               desc:"Trigger FERN_GLOW44 at max streak",     icon:"🌿", xp:124 }},
  {{ id:"{PW2.lower()}_use",       label:"Twig Glow",                   desc:"Trigger TWIG_GLOW44 power-up",          icon:"🪵", xp:62 }},
  {{ id:"{PW2.lower()}_max",       label:"Twig Glow Max",               desc:"Trigger TWIG_GLOW44 at max streak",     icon:"🪵", xp:124 }},
...
CMT = f'''// {PW1} — +{PW1_SC} fern glow bonus
    // {PW2} — +{PW2_SC} twig glow bonus
    // {PREV_PW1}'''
...
DRAWS = f'''function drawTinaksiteFox9e(ctx,r,ts,sp){{
  const p=ts/sp,pu=p<0.5?p*2:2-p*2;
  ctx.save();
  ctx.shadowColor="{FOX_GLOW}";ctx.shadowBlur=20+pu*18;
  ctx.beginPath();ctx.arc(0,0,r,0,Math.PI*2);
  const g=ctx.createRadialGradient(0,0,r*0.1,0,0,r);
  g.addColorStop(0,"#fed7aa");g.addColorStop(0.5,"{FOX_COLOR}");g.addColorStop(1,"#9a3412");
...
function drawTacharaniteOrb9e(ctx,r,ts,tp){{
  const p=ts/tp,pu=p<0.5?p*2:2-p*2;
  ctx.save();
  ctx.shadowColor="{ORB_GLOW}";ctx.shadowBlur=18+pu*16;
  ctx.beginPath();ctx.arc(0,0,r,0,Math.PI*2);
  const g=ctx.createRadialGradient(0,0,r*0.1,0,0,r);
  g.addColorStop(0,"#bae6fd");g.addColorStop(0.5,"{ORB_COLOR}");g.addColorStop(1,"#075985");
  ctx.fillStyle=g;ctx.fill();
...
DISP = f'''  else if(t.type==="{FOX_TYPE}"){{drawTinaksiteFox9e(ctx,t.radius,ts-t.born,t.lifetime);}}
  else if(t.type==="{ORB_TYPE}"){{drawTacharaniteOrb9e(ctx,t.radius,ts-t.born,t.lifetime);}}
...
with open(SRC, "w") as f:
    f.write(src)
print(f"Batch 812 done! +{len(src)-original_len} bytes")
