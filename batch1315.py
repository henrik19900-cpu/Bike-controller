...
PREV_PW1  = "ALPHA40_GLOW44"; PREV_PW1_ICON = "α\U0001fab4"
PREV_DRAW_FOX = "draw" + "Zincoquercetinaite" + "Fox9e"
PREV_FOX_R = "BASE_R*7.59"

FOX_FN = "draw" + "Zincoravnicite" + "Fox9e"
ORB_FN = "draw" + "Zincorolandite" + "Orb9e"

ACH = f'''  {{ id:"{FOX_TYPE}_tap",          label:"Zincoravnicite Fox Tap",      desc:"Tap Zincoravnicite Fox target",         icon:"{FOX_PEAK}", xp:62 }},
...
HANDLER = f'''}} else if(ptype==="{PW1}"){{
      ...
      showNotif("β BETA40 GLOW +"+bns);
      unlock("{PW1.lower()}_use");if(gs.streak>=20)unlock("{PW1.lower()}_max");
    }} else if(ptype==="{PW2}"){{
      const bns={PW2_SC}+gs.streak*12;
      gs.score+=bns;setHud(h=>({{...h,score:gs.score}}));
      spawnParticles(W/2,H/2,"{PW2_SHOCK}",28,"shockwave");
      showNotif("γ GAMMA40 GLOW +"+bns);
      unlock("{PW2.lower()}_use");if(gs.streak>=20)unlock("{PW2.lower()}_max");
    }} else if(ptype==="{PREV_PW1}")''' + '{'
OLD3 = f'}} else if(ptype==="{PREV_PW1}")' + '{'
assert src.count(OLD3) == 1; src = src.replace(OLD3, HANDLER, 1); print("OK Step3")

CMT = f'// {PW1} — +{PW1_SC} beta40\n    // {PW2} — +{PW2_SC} gamma40\n    // {PREV_PW1}'
...
print(f"Batch 1315 done! +{len(src)-original_len} bytes")
