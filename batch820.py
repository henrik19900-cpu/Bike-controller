...
FOX_COLOR = "#7dd3fc"; FOX_GLOW = "#f0f9ff"; FOX_PEAK = "🔵"
ORB_COLOR = "#fbbf24"; ORB_GLOW = "#fffbeb"; ORB_PEAK = "🌟"
FOX_SC   = 624;  ORB_SC  = 619
FOX_R    = "BASE_R*2.65"; ORB_R = "BASE_R*2.64"
OSC      = 0.4529; SW = 2434
PW1 = "WAVE2_GLOW44"; PW1_SC = 2744; PW1_SHOCK = "#0284c7"; PW1_ICON = "🌊💫"
PW2 = "MIST2_GLOW44"; PW2_SC = 2746; PW2_SHOCK = "#0369a1"; PW2_ICON = "🌫️✨"

# Prev-batch anchors (batch 819)
PREV_FOX_FULL = "wurtzite_fox9e"
PREV_FOX_COLOR = "#fef08a"; PREV_FOX_GLOW = "#fefce8"
PREV_PW1  = "DAWN2_GLOW44"; PREV_PW1_ICON = "🌅💫"
PREV_DRAW_FOX = "drawWurtziteFox9e"
PREV_FOX_R = "BASE_R*2.64"

# ── Step 1 – 8 achievements ───────────────────────────────────────────────────
ACH = f'''  {{ id:"{FOX_TYPE}_tap",          label:"Zunyite Fox Tap",             desc:"Tap Zunyite Fox target",                icon:"{FOX_PEAK}", xp:62 }},
  {{ id:"{FOX_TYPE}_peak",         label:"Zunyite Fox Peak",            desc:"Reach peak with Zunyite Fox",           icon:"{FOX_PEAK}", xp:124 }},
  {{ id:"{ORB_TYPE}_tap",          label:"Abernathyite Orb Tap",        desc:"Tap Abernathyite Orb target",           icon:"{ORB_PEAK}", xp:62 }},
  {{ id:"{ORB_TYPE}_peak",         label:"Abernathyite Orb Peak",       desc:"Reach peak with Abernathyite Orb",      icon:"{ORB_PEAK}", xp:124 }},
  {{ id:"{PW1.lower()}_use",       label:"Wave2 Glow",                  desc:"Trigger WAVE2_GLOW44 power-up",         icon:"🌊", xp:62 }},
  {{ id:"{PW1.lower()}_max",       label:"Wave2 Glow Max",              desc:"Trigger WAVE2_GLOW44 at max streak",    icon:"🌊", xp:124 }},
  {{ id:"{PW2.lower()}_use",       label:"Mist2 Glow",                  desc:"Trigger MIST2_GLOW44 power-up",         icon:"🌫️", xp:62 }},
  {{ id:"{PW2.lower()}_max",       label:"Mist2 Glow Max",              desc:"Trigger MIST2_GLOW44 at max streak",    icon:"🌫️", xp:124 }},
...
CMT = f'''// {PW1} — +{PW1_SC} wave2 glow bonus
    // {PW2} — +{PW2_SC} mist2 glow bonus
    // {PREV_PW1}'''
...
DRAWS = f'''function drawZunyiteFox9e(ctx,r,ts,sp){{
  const p=ts/sp,pu=p<0.5?p*2:2-p*2;
  ctx.save();
  ctx.shadowColor="{FOX_GLOW}";ctx.shadowBlur=20+pu*18;
  ctx.beginPath();ctx.arc(0,0,r,0,Math.PI*2);
  const g=ctx.createRadialGradient(0,0,r*0.1,0,0,r);
  g.addColorStop(0,"#e0f2fe");g.addColorStop(0.5,"{FOX_COLOR}");g.addColorStop(1,"#075985");
...
function drawAbernathyiteOrb9e(ctx,r,ts,tp){{
  const p=ts/tp,pu=p<0.5?p*2:2-p*2;
  ctx.save();
  ctx.shadowColor="{ORB_GLOW}";ctx.shadowBlur=18+pu*16;
  ctx.beginPath();ctx.arc(0,0,r,0,Math.PI*2);
  const g=ctx.createRadialGradient(0,0,r*0.1,0,0,r);
  g.addColorStop(0,"#fde68a");g.addColorStop(0.5,"{ORB_COLOR}");g.addColorStop(1,"#92400e");
  ctx.fillStyle=g;ctx.fill();
...
DISP = f'''  else if(t.type==="{FOX_TYPE}"){{drawZunyiteFox9e(ctx,t.radius,ts-t.born,t.lifetime);}}
  else if(t.type==="{ORB_TYPE}"){{drawAbernathyiteOrb9e(ctx,t.radius,ts-t.born,t.lifetime);}}
...
with open(SRC, "w") as f:
    f.write(src)
print(f"Batch 820 done! +{len(src)-original_len} bytes")
