...
FOX_COLOR = "#ec4899"; FOX_GLOW = "#fdf2f8"; FOX_PEAK = "💗"
ORB_COLOR = "#10b981"; ORB_GLOW = "#ecfdf5"; ORB_PEAK = "🌈"
FOX_SC   = 630;  ORB_SC  = 625
FOX_R    = "BASE_R*2.68"; ORB_R = "BASE_R*2.67"
OSC      = 0.4541; SW = 2440
PW1 = "CREST2_GLOW44"; PW1_SC = 2756; PW1_SHOCK = "#0284c7"; PW1_ICON = "🏄💫"
PW2 = "SWELL_GLOW44";  PW2_SC = 2758; PW2_SHOCK = "#0369a1"; PW2_ICON = "🌊✨"

# Prev-batch anchors (batch 822)
PREV_FOX_FULL = "aerinite_fox9e"
PREV_FOX_COLOR = "#3b82f6"; PREV_FOX_GLOW = "#eff6ff"
PREV_PW1  = "REEF2_GLOW44"; PREV_PW1_ICON = "🐠💫"
PREV_DRAW_FOX = "drawAeriniteFox9e"
PREV_FOX_R = "BASE_R*2.67"

# ── Step 1 – 8 achievements ───────────────────────────────────────────────────
ACH = f'''  {{ id:"{FOX_TYPE}_tap",          label:"Alleghanyite Fox Tap",        desc:"Tap Alleghanyite Fox target",           icon:"{FOX_PEAK}", xp:62 }},
  {{ id:"{FOX_TYPE}_peak",         label:"Alleghanyite Fox Peak",       desc:"Reach peak with Alleghanyite Fox",      icon:"{FOX_PEAK}", xp:124 }},
  {{ id:"{ORB_TYPE}_tap",          label:"Ammiolite Orb Tap",           desc:"Tap Ammiolite Orb target",              icon:"{ORB_PEAK}", xp:62 }},
  {{ id:"{ORB_TYPE}_peak",         label:"Ammiolite Orb Peak",          desc:"Reach peak with Ammiolite Orb",         icon:"{ORB_PEAK}", xp:124 }},
  {{ id:"{PW1.lower()}_use",       label:"Crest2 Glow",                 desc:"Trigger CREST2_GLOW44 power-up",        icon:"🏄", xp:62 }},
  {{ id:"{PW1.lower()}_max",       label:"Crest2 Glow Max",             desc:"Trigger CREST2_GLOW44 at max streak",   icon:"🏄", xp:124 }},
  {{ id:"{PW2.lower()}_use",       label:"Swell Glow",                  desc:"Trigger SWELL_GLOW44 power-up",         icon:"🌊", xp:62 }},
  {{ id:"{PW2.lower()}_max",       label:"Swell Glow Max",              desc:"Trigger SWELL_GLOW44 at max streak",    icon:"🌊", xp:124 }},
...
CMT = f'''// {PW1} — +{PW1_SC} crest2 glow bonus
    // {PW2} — +{PW2_SC} swell glow bonus
    // {PREV_PW1}'''
...
DRAWS = f'''function drawAlleghanyiteFox9e(ctx,r,ts,sp){{
  const p=ts/sp,pu=p<0.5?p*2:2-p*2;
  ctx.save();
  ctx.shadowColor="{FOX_GLOW}";ctx.shadowBlur=20+pu*18;
  ctx.beginPath();ctx.arc(0,0,r,0,Math.PI*2);
  const g=ctx.createRadialGradient(0,0,r*0.1,0,0,r);
  g.addColorStop(0,"#fce7f3");g.addColorStop(0.5,"{FOX_COLOR}");g.addColorStop(1,"#831843");
...
function drawAmmioliteOrb9e(ctx,r,ts,tp){{
  const p=ts/tp,pu=p<0.5?p*2:2-p*2;
  ctx.save();
  ctx.shadowColor="{ORB_GLOW}";ctx.shadowBlur=18+pu*16;
  ctx.beginPath();ctx.arc(0,0,r,0,Math.PI*2);
  const g=ctx.createRadialGradient(0,0,r*0.1,0,0,r);
  g.addColorStop(0,"#6ee7b7");g.addColorStop(0.5,"{ORB_COLOR}");g.addColorStop(1,"#064e3b");
  ctx.fillStyle=g;ctx.fill();
...
DISP = f'''  else if(t.type==="{FOX_TYPE}"){{drawAlleghanyiteFox9e(ctx,t.radius,ts-t.born,t.lifetime);}}
  else if(t.type==="{ORB_TYPE}"){{drawAmmioliteOrb9e(ctx,t.radius,ts-t.born,t.lifetime);}}
...
with open(SRC, "w") as f:
    f.write(src)
print(f"Batch 823 done! +{len(src)-original_len} bytes")
