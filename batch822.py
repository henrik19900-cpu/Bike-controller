...
FOX_COLOR = "#3b82f6"; FOX_GLOW = "#eff6ff"; FOX_PEAK = "💙"
ORB_COLOR = "#c084fc"; ORB_GLOW = "#faf5ff"; ORB_PEAK = "💜"
FOX_SC   = 628;  ORB_SC  = 623
FOX_R    = "BASE_R*2.67"; ORB_R = "BASE_R*2.66"
OSC      = 0.4537; SW = 2438
PW1 = "REEF2_GLOW44";  PW1_SC = 2752; PW1_SHOCK = "#0891b2"; PW1_ICON = "🐠💫"
PW2 = "GALE2_GLOW44";  PW2_SC = 2754; PW2_SHOCK = "#0e7490"; PW2_ICON = "💨✨"

# Prev-batch anchors (batch 821)
PREV_FOX_FULL = "abswurmbachite_fox9e"
PREV_FOX_COLOR = "#f43f5e"; PREV_FOX_GLOW = "#fff1f2"
PREV_PW1  = "HAZE2_GLOW44"; PREV_PW1_ICON = "🔮💫"
PREV_DRAW_FOX = "drawAbswurmbachiteFox9e"
PREV_FOX_R = "BASE_R*2.66"

# ── Step 1 – 8 achievements ───────────────────────────────────────────────────
ACH = f'''  {{ id:"{FOX_TYPE}_tap",          label:"Aerinite Fox Tap",            desc:"Tap Aerinite Fox target",               icon:"{FOX_PEAK}", xp:62 }},
  {{ id:"{FOX_TYPE}_peak",         label:"Aerinite Fox Peak",           desc:"Reach peak with Aerinite Fox",          icon:"{FOX_PEAK}", xp:124 }},
  {{ id:"{ORB_TYPE}_tap",          label:"Allabogdanite Orb Tap",       desc:"Tap Allabogdanite Orb target",          icon:"{ORB_PEAK}", xp:62 }},
  {{ id:"{ORB_TYPE}_peak",         label:"Allabogdanite Orb Peak",      desc:"Reach peak with Allabogdanite Orb",     icon:"{ORB_PEAK}", xp:124 }},
  {{ id:"{PW1.lower()}_use",       label:"Reef2 Glow",                  desc:"Trigger REEF2_GLOW44 power-up",         icon:"🐠", xp:62 }},
  {{ id:"{PW1.lower()}_max",       label:"Reef2 Glow Max",              desc:"Trigger REEF2_GLOW44 at max streak",    icon:"🐠", xp:124 }},
  {{ id:"{PW2.lower()}_use",       label:"Gale2 Glow",                  desc:"Trigger GALE2_GLOW44 power-up",         icon:"💨", xp:62 }},
  {{ id:"{PW2.lower()}_max",       label:"Gale2 Glow Max",              desc:"Trigger GALE2_GLOW44 at max streak",    icon:"💨", xp:124 }},
...
CMT = f'''// {PW1} — +{PW1_SC} reef2 glow bonus
    // {PW2} — +{PW2_SC} gale2 glow bonus
    // {PREV_PW1}'''
...
DRAWS = f'''function drawAeriniteFox9e(ctx,r,ts,sp){{
  const p=ts/sp,pu=p<0.5?p*2:2-p*2;
  ctx.save();
  ctx.shadowColor="{FOX_GLOW}";ctx.shadowBlur=20+pu*18;
  ctx.beginPath();ctx.arc(0,0,r,0,Math.PI*2);
  const g=ctx.createRadialGradient(0,0,r*0.1,0,0,r);
  g.addColorStop(0,"#bfdbfe");g.addColorStop(0.5,"{FOX_COLOR}");g.addColorStop(1,"#1e3a8a");
...
function drawAllabogdaniteOrb9e(ctx,r,ts,tp){{
  const p=ts/tp,pu=p<0.5?p*2:2-p*2;
  ctx.save();
  ctx.shadowColor="{ORB_GLOW}";ctx.shadowBlur=18+pu*16;
  ctx.beginPath();ctx.arc(0,0,r,0,Math.PI*2);
  const g=ctx.createRadialGradient(0,0,r*0.1,0,0,r);
  g.addColorStop(0,"#e9d5ff");g.addColorStop(0.5,"{ORB_COLOR}");g.addColorStop(1,"#4c1d95");
  ctx.fillStyle=g;ctx.fill();
...
DISP = f'''  else if(t.type==="{FOX_TYPE}"){{drawAeriniteFox9e(ctx,t.radius,ts-t.born,t.lifetime);}}
  else if(t.type==="{ORB_TYPE}"){{drawAllabogdaniteOrb9e(ctx,t.radius,ts-t.born,t.lifetime);}}
...
with open(SRC, "w") as f:
    f.write(src)
print(f"Batch 822 done! +{len(src)-original_len} bytes")
