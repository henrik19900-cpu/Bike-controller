...
FOX_COLOR = "#1c1917"; FOX_GLOW = "#e7e5e4"; FOX_PEAK = "☢️"
ORB_COLOR = "#155e75"; ORB_GLOW = "#cffafe"; ORB_PEAK = "🔷"
FOX_SC   = 618;  ORB_SC  = 613
FOX_R    = "BASE_R*2.62"; ORB_R = "BASE_R*2.61"
OSC      = 0.4517; SW = 2428
PW1 = "CLAY_GLOW44"; PW1_SC = 2732; PW1_SHOCK = "#92400e"; PW1_ICON = "🏺💫"
PW2 = "SAND_GLOW44"; PW2_SC = 2734; PW2_SHOCK = "#b45309"; PW2_ICON = "🏜️✨"

# Prev-batch anchors (batch 816)
PREV_FOX_FULL = "uralite_fox9e"
PREV_FOX_COLOR = "#0f766e"; PREV_FOX_GLOW = "#ccfbf1"
PREV_PW1  = "KELP_GLOW44"; PREV_PW1_ICON = "🌊💫"
PREV_DRAW_FOX = "drawUraliteFox9e"
PREV_FOX_R = "BASE_R*2.61"

# ── Step 1 – 8 achievements ───────────────────────────────────────────────────
ACH = f'''  {{ id:"{FOX_TYPE}_tap",          label:"Uraninite Fox Tap",           desc:"Tap Uraninite Fox target",              icon:"{FOX_PEAK}", xp:62 }},
  {{ id:"{FOX_TYPE}_peak",         label:"Uraninite Fox Peak",          desc:"Reach peak with Uraninite Fox",         icon:"{FOX_PEAK}", xp:124 }},
  {{ id:"{ORB_TYPE}_tap",          label:"Wairakite Orb Tap",           desc:"Tap Wairakite Orb target",              icon:"{ORB_PEAK}", xp:62 }},
  {{ id:"{ORB_TYPE}_peak",         label:"Wairakite Orb Peak",          desc:"Reach peak with Wairakite Orb",         icon:"{ORB_PEAK}", xp:124 }},
  {{ id:"{PW1.lower()}_use",       label:"Clay Glow",                   desc:"Trigger CLAY_GLOW44 power-up",          icon:"🏺", xp:62 }},
  {{ id:"{PW1.lower()}_max",       label:"Clay Glow Max",               desc:"Trigger CLAY_GLOW44 at max streak",     icon:"🏺", xp:124 }},
  {{ id:"{PW2.lower()}_use",       label:"Sand Glow",                   desc:"Trigger SAND_GLOW44 power-up",          icon:"🏜️", xp:62 }},
  {{ id:"{PW2.lower()}_max",       label:"Sand Glow Max",               desc:"Trigger SAND_GLOW44 at max streak",     icon:"🏜️", xp:124 }},
...
CMT = f'''// {PW1} — +{PW1_SC} clay glow bonus
    // {PW2} — +{PW2_SC} sand glow bonus
    // {PREV_PW1}'''
...
DRAWS = f'''function drawUraniniteF9e(ctx,r,ts,sp){{
  const p=ts/sp,pu=p<0.5?p*2:2-p*2;
  ctx.save();
  ctx.shadowColor="{FOX_GLOW}";ctx.shadowBlur=20+pu*18;
  ctx.beginPath();ctx.arc(0,0,r,0,Math.PI*2);
  const g=ctx.createRadialGradient(0,0,r*0.1,0,0,r);
  g.addColorStop(0,"#78716c");g.addColorStop(0.5,"{FOX_COLOR}");g.addColorStop(1,"#000000");
...
function drawWairakiteOrb9e(ctx,r,ts,tp){{
  const p=ts/tp,pu=p<0.5?p*2:2-p*2;
  ctx.save();
  ctx.shadowColor="{ORB_GLOW}";ctx.shadowBlur=18+pu*16;
  ctx.beginPath();ctx.arc(0,0,r,0,Math.PI*2);
  const g=ctx.createRadialGradient(0,0,r*0.1,0,0,r);
  g.addColorStop(0,"#a5f3fc");g.addColorStop(0.5,"{ORB_COLOR}");g.addColorStop(1,"#083344");
  ctx.fillStyle=g;ctx.fill();
...
DISP = f'''  else if(t.type==="{FOX_TYPE}"){{drawUraniniteF9e(ctx,t.radius,ts-t.born,t.lifetime);}}
  else if(t.type==="{ORB_TYPE}"){{drawWairakiteOrb9e(ctx,t.radius,ts-t.born,t.lifetime);}}
...
with open(SRC, "w") as f:
    f.write(src)
print(f"Batch 817 done! +{len(src)-original_len} bytes")
