...
ORB_C = "#9333ea"; ORB_G = "#faf5ff"
FOX_L = "#86efac"; FOX_M = "#22c55e"; FOX_D = "#052e16"
FOX_RL,FOX_RG,FOX_RB = 134,239,172
ORB_L = "#d8b4fe"; ORB_M = "#a855f7"; ORB_D = "#3b0764"
ORB_RL,ORB_RG,ORB_RB = 216,180,254
PW1C = "#6ee7b7"; PW2C = "#67e8f9"
PEAK_FOX = "🐠"; PEAK_ORB = "🪸"

# STEP 1
A1_ANCHOR = '{ id:"gowerite_fox9e_tap", label:"Gowerite Fox Tap"'
assert A1_ANCHOR in code, "S1 anchor missing"
A1_NEW = (
  '{ id:"groutite_fox9e_tap", label:"Groutite Fox Tap", desc:"Tap Groutite Fox", icon:"🦊", xp:62 },\n'
  '  { id:"groutite_fox9e_peak", label:"Groutite Fox Peak", desc:"Hit Groutite Fox at peak", icon:"🐠", xp:78 },\n'
  '  { id:"grunerite_orb9e_tap", label:"Grunerite Orb Tap", desc:"Tap Grunerite Orb", icon:"🔮", xp:60 },\n'
  '  { id:"grunerite_orb9e_peak", label:"Grunerite Orb Peak", desc:"Hit Grunerite Orb at peak", icon:"🪸", xp:76 },\n'
  '  { id:"reef_glow44_use", label:"Reef Glow Use", desc:"Collect Reef Glow power-up", icon:"🐠", xp:44 },\n'
  '  { id:"reef_glow44_max", label:"Reef Glow Max", desc:"Collect 5 Reef Glow power-ups", icon:"🐠", xp:88 },\n'
  '  { id:"atoll_glow44_use", label:"Atoll Glow Use", desc:"Collect Atoll Glow power-up", icon:"🏝️", xp:44 },\n'
  '  { id:"atoll_glow44_max", label:"Atoll Glow Max", desc:"Collect 5 Atoll Glow power-ups", icon:"🏝️", xp:88 },\n'
  '  { id:"gowerite_fox9e_tap", label:"Gowerite Fox Tap"'
)
code = code.replace(A1_ANCHOR, A1_NEW, 1)
assert '{ id:"groutite_fox9e_tap"' in code, "S1 failed"
print("S1 ok")

# STEP 2
A2_ANCHOR = '"DELTA_GLOW44","LAGOON_GLOW44","OASIS_GLOW44"'
assert A2_ANCHOR in code, "S2 anchor missing"
A2_NEW = f'"{PW1}","{PW2}","DELTA_GLOW44","LAGOON_GLOW44","OASIS_GLOW44"'
code = code.replace(A2_ANCHOR, A2_NEW, 1)
assert f'"{PW1}"' in code, "S2 failed"
print("S2 ok")

# STEP 3
A3_ANCHOR = '} else if(ptype==="DELTA_GLOW44"){'
...
# STEP 4
A4_ANCHOR = '// DELTA_GLOW44 — +2516 delta glow bonus'
assert A4_ANCHOR in code, "S4 anchor missing"
A4_NEW = (
  f'// {PW1} — +{PW1SC} reef glow bonus\n'
  f'      // {PW2} — +{PW2SC} atoll glow bonus\n'
  f'      // DELTA_GLOW44 — +2516 delta glow bonus'
)
...
# STEP 5
A5_ANCHOR = 'function drawGoweriteFox9e('
assert A5_ANCHOR in code, "S5 anchor missing"
DRAW = (
f'function drawGroutiteFox9e(ctx,r,ts,sp){{\n'
...
f'  ctx.restore();\n'
f'}}\n'
f'function drawGruneriteOrb9e(ctx,r,ts,tp){{\n'
...
f'  ctx.restore();\n'
f'}}\n'
f'function drawGoweriteFox9e('
)
code = code.replace(A5_ANCHOR, DRAW, 1)
assert 'function drawGroutiteFox9e(' in code, "S5 failed"
print("S5 ok")

# STEP 6
A6_ANCHOR = '  else if(t.type==="gowerite_fox9e"){'
...
f'    drawGroutiteFox9e(ctx,t.radius,ts,sp2);ctx.restore();\n'
f'  }}\n'
f'  else if(t.type==="{ORB}"){{\n'
f'    ctx.save();ctx.translate(t.x,t.y);\n'
f'    const tp2=Math.min(1,(ts-t.born)/{SW});\n'
f'    drawGruneriteOrb9e(ctx,t.radius,ts,tp2);ctx.restore();\n'
f'  }}\n'
f'  else if(t.type==="gowerite_fox9e"){{'
)
...
# STEP 7
A7_ANCHOR = 'if(hit.type==="gowerite_fox9e"){'
...
f'      if(isPeak)unlock("groutite_fox9e_peak");\n'
f'      unlock("groutite_fox9e_tap");\n'
...
f'      addParticle(hit.x,hit.y,"popup",isPeak?"PEAK! +"+pts:"🔮 +"+pts);\n'
f'      if(isPeak)unlock("grunerite_orb9e_peak");\n'
f'      unlock("grunerite_orb9e_tap");\n'
f'      updateMissions({{...gs.sessionStats,rareHits:gs.sessionStats.rareHits}});\n'
f'      sfx("epic");vibrate([40,10,20]);\n'
f'    }}else if(hit.type==="gowerite_fox9e"){{'
)
...
# STEP 8
A8_OLD = ('      type="gowerite_fox9e";color="#7e22ce";glow="#faf5ff";\n'
          '    } else if('+COND100F+'){\n'
          '      type="griphite_orb9e"')
...
A8_NEW = (
    ...
    f'    }} else if({COND100F}){{\n'
    f'      type="gowerite_fox9e";color="#7e22ce";glow="#faf5ff";\n'
    f'    }} else if({COND100F}){{\n'
    f'      type="griphite_orb9e"'
)
...
# STEP 9
A9_ANCHOR = 'type==="gowerite_fox9e"?BASE_R*2.08:type==="griphite_orb9e"?BASE_R*2.07:'
...
# STEP 10
A10_ANCHOR = 'DELTA_GLOW44:"🌊✨",LAGOON_GLOW44:"🏝️✨",OASIS_GLOW44:"🌴✨"'
cnt = code.count(A10_ANCHOR)
assert cnt == 2, f"S10 anchor count={cnt}, expected 2"
A10_NEW = f'{PW1}:"🐠✨",{PW2}:"🏝️✨",DELTA_GLOW44:"🌊✨",LAGOON_GLOW44:"🏝️✨",OASIS_GLOW44:"🌴✨"'
...
with open(SRC, "w") as f:
    f.write(code)
print(f"Batch 764 done! +{len(code)-orig_len} bytes")
