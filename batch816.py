...
ORB_COLOR = "#a8a29e"; ORB_GLOW = "#fafaf9"; ORB_PEAK = "💎"
FOX_SC   = 616;  ORB_SC  = 611
FOX_R    = "BASE_R*2.61"; ORB_R = "BASE_R*2.60"
OSC      = 0.4513; SW = 2426
PW1 = "KELP_GLOW44";  PW1_SC = 2728; PW1_SHOCK = "#0d9488"; PW1_ICON = "🌊💫"
PW2 = "ALGAE_GLOW44"; PW2_SC = 2730; PW2_SHOCK = "#0f766e"; PW2_ICON = "💧✨"

# Prev-batch anchors (batch 815)
PREV_FOX_FULL = "tschermigite_fox9e"
PREV_FOX_COLOR = "#e2e8f0"; PREV_FOX_GLOW = "#f8fafc"
PREV_PW1  = "CROP_GLOW44"; PREV_PW1_ICON = "🌽💫"
PREV_DRAW_FOX = "drawTschermigiteFox9e"
PREV_FOX_R = "BASE_R*2.60"

# ── Step 1 – 8 achievements ───────────────────────────────────────────────────
ACH = f'''  {{ id:"{FOX_TYPE}_tap",          label:"Uralite Fox Tap",             desc:"Tap Uralite Fox target",                icon:"{FOX_PEAK}", xp:62 }},
  {{ id:"{FOX_TYPE}_peak",         label:"Uralite Fox Peak",            desc:"Reach peak with Uralite Fox",           icon:"{FOX_PEAK}", xp:124 }},
  {{ id:"{ORB_TYPE}_tap",          label:"Valentinite Orb Tap",         desc:"Tap Valentinite Orb target",            icon:"{ORB_PEAK}", xp:62 }},
  {{ id:"{ORB_TYPE}_peak",         label:"Valentinite Orb Peak",        desc:"Reach peak with Valentinite Orb",       icon:"{ORB_PEAK}", xp:124 }},
  {{ id:"{PW1.lower()}_use",       label:"Kelp Glow",                   desc:"Trigger KELP_GLOW44 power-up",          icon:"🌊", xp:62 }},
  {{ id:"{PW1.lower()}_max",       label:"Kelp Glow Max",               desc:"Trigger KELP_GLOW44 at max streak",     icon:"🌊", xp:124 }},
  {{ id:"{PW2.lower()}_use",       label:"Algae Glow",                  desc:"Trigger ALGAE_GLOW44 power-up",         icon:"💧", xp:62 }},
  {{ id:"{PW2.lower()}_max",       label:"Algae Glow Max",              desc:"Trigger ALGAE_GLOW44 at max streak",    icon:"💧", xp:124 }},
...
CMT = f'''// {PW1} — +{PW1_SC} kelp glow bonus
    // {PW2} — +{PW2_SC} algae glow bonus
    // {PREV_PW1}'''
...
DRAWS = f'''function drawUraliteFox9e(ctx,r,ts,sp){{
  const p=ts/sp,pu=p<0.5?p*2:2-p*2;
  ctx.save();
  ctx.shadowColor="{FOX_GLOW}";ctx.shadowBlur=20+pu*18;
  ctx.beginPath();ctx.arc(0,0,r,0,Math.PI*2);
  const g=ctx.createRadialGradient(0,0,r*0.1,0,0,r);
  g.addColorStop(0,"#99f6e4");g.addColorStop(0.5,"{FOX_COLOR}");g.addColorStop(1,"#134e4a");
...
function drawValentiniteOrb9e(ctx,r,ts,tp){{
  const p=ts/tp,pu=p<0.5?p*2:2-p*2;
  ctx.save();
  ctx.shadowColor="{ORB_GLOW}";ctx.shadowBlur=18+pu*16;
  ctx.beginPath();ctx.arc(0,0,r,0,Math.PI*2);
  const g=ctx.createRadialGradient(0,0,r*0.1,0,0,r);
  g.addColorStop(0,"#f5f5f4");g.addColorStop(0.5,"{ORB_COLOR}");g.addColorStop(1,"#44403c");
  ctx.fillStyle=g;ctx.fill();
...
DISP = f'''  else if(t.type==="{FOX_TYPE}"){{drawUraliteFox9e(ctx,t.radius,ts-t.born,t.lifetime);}}
  else if(t.type==="{ORB_TYPE}"){{drawValentiniteOrb9e(ctx,t.radius,ts-t.born,t.lifetime);}}
...
with open(SRC, "w") as f:
    f.write(src)
print(f"Batch 816 done! +{len(src)-original_len} bytes")
