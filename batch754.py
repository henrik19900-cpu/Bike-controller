...
OSC   = "0.4265"
SW    = "2302"
FOX_C = "#dc2626"; FOX_G = "#fef2f2"
ORB_C = "#1e40af"; ORB_G = "#eff6ff"
FOX_L = "#fca5a5"; FOX_M = "#dc2626"; FOX_D = "#450a0a"
FOX_RL,FOX_RG,FOX_RB = 252,165,165
ORB_L = "#93c5fd"; ORB_M = "#2563eb"; ORB_D = "#1e3a8a"
ORB_RL,ORB_RG,ORB_RB = 147,197,253
PW1C = "#bae6fd"; PW2C = "#e0f2fe"
PEAK_FOX = "🔥"; PEAK_ORB = "❄️"

# STEP 1
A1_ANCHOR = '{ id:"dioptase_fox9e_tap", label:"Dioptase Fox Tap"'
assert A1_ANCHOR in code, "S1 anchor missing"
A1_NEW = (
  '{ id:"durangite_fox9e_tap", label:"Durangite Fox Tap", desc:"Tap Durangite Fox", icon:"🦊", xp:62 },\n'
  '  { id:"durangite_fox9e_peak", label:"Durangite Fox Peak", desc:"Hit Durangite Fox at peak", icon:"🔥", xp:78 },\n'
  '  { id:"dysanalyte_orb9e_tap", label:"Dysanalyte Orb Tap", desc:"Tap Dysanalyte Orb", icon:"🔮", xp:60 },\n'
  '  { id:"dysanalyte_orb9e_peak", label:"Dysanalyte Orb Peak", desc:"Hit Dysanalyte Orb at peak", icon:"❄️", xp:76 },\n'
  '  { id:"sleet_glow44_use", label:"Sleet Glow Use", desc:"Collect Sleet Glow power-up", icon:"🌨️", xp:44 },\n'
  '  { id:"sleet_glow44_max", label:"Sleet Glow Max", desc:"Collect 5 Sleet Glow power-ups", icon:"🌨️", xp:88 },\n'
  '  { id:"snow_glow44_use", label:"Snow Glow Use", desc:"Collect Snow Glow power-up", icon:"❄️", xp:44 },\n'
  '  { id:"snow_glow44_max", label:"Snow Glow Max", desc:"Collect 5 Snow Glow power-ups", icon:"❄️", xp:88 },\n'
  '  { id:"dioptase_fox9e_tap", label:"Dioptase Fox Tap"'
)
code = code.replace(A1_ANCHOR, A1_NEW, 1)
assert '{ id:"durangite_fox9e_tap"' in code, "S1 failed"
print("S1 ok")

# STEP 2
A2_ANCHOR = '"HAZE_GLOW44","RAIN_GLOW44","CLOUD_GLOW44"'
assert A2_ANCHOR in code, "S2 anchor missing"
A2_NEW = f'"{PW1}","{PW2}","HAZE_GLOW44","RAIN_GLOW44","CLOUD_GLOW44"'
code = code.replace(A2_ANCHOR, A2_NEW, 1)
assert f'"{PW1}"' in code, "S2 failed"
print("S2 ok")

# STEP 3
A3_ANCHOR = '} else if(ptype==="HAZE_GLOW44"){'
...
# STEP 4
A4_ANCHOR = '// HAZE_GLOW44 — +2476 haze glow bonus'
assert A4_ANCHOR in code, "S4 anchor missing"
A4_NEW = (
  f'// {PW1} — +{PW1SC} sleet glow bonus\n'
  f'      // {PW2} — +{PW2SC} snow glow bonus\n'
  f'      // HAZE_GLOW44 — +2476 haze glow bonus'
)
...
# STEP 5
A5_ANCHOR = 'function drawDioptaseFox9e('
assert A5_ANCHOR in code, "S5 anchor missing"
DRAW = (
f'function drawDurangiteFox9e(ctx,r,ts,sp){{\n'
...
f'  ctx.restore();\n'
f'}}\n'
f'function drawDysanalyteOrb9e(ctx,r,ts,tp){{\n'
...
f'  ctx.restore();\n'
f'}}\n'
f'function drawDioptaseFox9e('
)
code = code.replace(A5_ANCHOR, DRAW, 1)
assert 'function drawDurangiteFox9e(' in code, "S5 failed"
print("S5 ok")

# STEP 6
A6_ANCHOR = '  else if(t.type==="dioptase_fox9e"){'
...
f'    drawDurangiteFox9e(ctx,t.radius,ts,sp2);ctx.restore();\n'
f'  }}\n'
f'  else if(t.type==="{ORB}"){{\n'
f'    ctx.save();ctx.translate(t.x,t.y);\n'
f'    const tp2=Math.min(1,(ts-t.born)/{SW});\n'
f'    drawDysanalyteOrb9e(ctx,t.radius,ts,tp2);ctx.restore();\n'
f'  }}\n'
f'  else if(t.type==="dioptase_fox9e"){{'
)
...
# STEP 7
A7_ANCHOR = 'if(hit.type==="dioptase_fox9e"){'
...
f'      if(isPeak)unlock("durangite_fox9e_peak");\n'
f'      unlock("durangite_fox9e_tap");\n'
...
f'      addParticle(hit.x,hit.y,"popup",isPeak?"PEAK! +"+pts:"🔮 +"+pts);\n'
f'      if(isPeak)unlock("dysanalyte_orb9e_peak");\n'
f'      unlock("dysanalyte_orb9e_tap");\n'
f'      updateMissions({{...gs.sessionStats,rareHits:gs.sessionStats.rareHits}});\n'
f'      sfx("epic");vibrate([40,10,20]);\n'
f'    }}else if(hit.type==="dioptase_fox9e"){{'
)
...
# STEP 8
A8_OLD = ('      type="dioptase_fox9e";color="#166534";glow="#f0fdf4";\n'
          '    } else if('+COND100F+'){\n'
          '      type="domeykite_orb9e"')
...
A8_NEW = (
    ...
    f'    }} else if({COND100F}){{\n'
    f'      type="dioptase_fox9e";color="#166534";glow="#f0fdf4";\n'
    f'    }} else if({COND100F}){{\n'
    f'      type="domeykite_orb9e"'
)
...
# STEP 9
A9_ANCHOR = 'type==="dioptase_fox9e"?BASE_R*1.98:type==="domeykite_orb9e"?BASE_R*1.97:'
...
# STEP 10
A10_ANCHOR = 'HAZE_GLOW44:"🌫️✨",RAIN_GLOW44:"🌧️✨",CLOUD_GLOW44:"☁️✨"'
cnt = code.count(A10_ANCHOR)
assert cnt == 2, f"S10 anchor count={cnt}, expected 2"
A10_NEW = f'{PW1}:"🌨️✨",{PW2}:"❄️✨",HAZE_GLOW44:"🌫️✨",RAIN_GLOW44:"🌧️✨",CLOUD_GLOW44:"☁️✨"'
...
with open(SRC, "w") as f:
    f.write(code)
print(f"Batch 754 done! +{len(code)-orig_len} bytes")
