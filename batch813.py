...
FOX_COLOR = "#d97706"; FOX_GLOW = "#fef9c3"; FOX_PEAK = "🔸"
ORB_COLOR = "#374151"; ORB_GLOW = "#f9fafb"; ORB_PEAK = "⬛"
FOX_SC   = 610;  ORB_SC  = 605
FOX_R    = "BASE_R*2.58"; ORB_R = "BASE_R*2.57"
OSC      = 0.4501; SW = 2420
PW1 = "STEM_GLOW44";  PW1_SC = 2716; PW1_SHOCK = "#0ea5e9"; PW1_ICON = "💧💫"
PW2 = "HERB_GLOW44";  PW2_SC = 2718; PW2_SHOCK = "#0284c7"; PW2_ICON = "🌊✨"

# Prev-batch anchors (batch 812)
PREV_FOX_FULL = "tinaksite_fox9e"
PREV_FOX_COLOR = "#ea580c"; PREV_FOX_GLOW = "#fff7ed"
PREV_PW1  = "FERN_GLOW44"; PREV_PW1_ICON = "🌿💫"
PREV_DRAW_FOX = "drawTinaksiteFox9e"
PREV_FOX_R = "BASE_R*2.57"

# ── Step 1 – 8 achievements ───────────────────────────────────────────────────
ACH = f'''  {{ id:"{FOX_TYPE}_tap",          label:"Tochilinite Fox Tap",         desc:"Tap Tochilinite Fox target",            icon:"{FOX_PEAK}", xp:62 }},
  {{ id:"{FOX_TYPE}_peak",         label:"Tochilinite Fox Peak",        desc:"Reach peak with Tochilinite Fox",       icon:"{FOX_PEAK}", xp:124 }},
  {{ id:"{ORB_TYPE}_tap",          label:"Tomichite Orb Tap",           desc:"Tap Tomichite Orb target",              icon:"{ORB_PEAK}", xp:62 }},
  {{ id:"{ORB_TYPE}_peak",         label:"Tomichite Orb Peak",          desc:"Reach peak with Tomichite Orb",         icon:"{ORB_PEAK}", xp:124 }},
  {{ id:"{PW1.lower()}_use",       label:"Stem Glow",                   desc:"Trigger STEM_GLOW44 power-up",          icon:"💧", xp:62 }},
  {{ id:"{PW1.lower()}_max",       label:"Stem Glow Max",               desc:"Trigger STEM_GLOW44 at max streak",     icon:"💧", xp:124 }},
  {{ id:"{PW2.lower()}_use",       label:"Herb Glow",                   desc:"Trigger HERB_GLOW44 power-up",          icon:"🌊", xp:62 }},
  {{ id:"{PW2.lower()}_max",       label:"Herb Glow Max",               desc:"Trigger HERB_GLOW44 at max streak",     icon:"🌊", xp:124 }},
...
CMT = f'''// {PW1} — +{PW1_SC} stem glow bonus
    // {PW2} — +{PW2_SC} herb glow bonus
    // {PREV_PW1}'''
...
DRAWS = f'''function drawTochiliniteF9e(ctx,r,ts,sp){{
...
function drawTomichiteOrb9e(ctx,r,ts,tp){{
  const p=ts/tp,pu=p<0.5?p*2:2-p*2;
  ctx.save();
  ctx.shadowColor="{ORB_GLOW}";ctx.shadowBlur=18+pu*16;
  ctx.beginPath();ctx.arc(0,0,r,0,Math.PI*2);
  const g=ctx.createRadialGradient(0,0,r*0.1,0,0,r);
  g.addColorStop(0,"#d1d5db");g.addColorStop(0.5,"{ORB_COLOR}");g.addColorStop(1,"#111827");
  ctx.fillStyle=g;ctx.fill();
...
DISP = f'''  else if(t.type==="{FOX_TYPE}"){{drawTochiliniteF9e(ctx,t.radius,ts-t.born,t.lifetime);}}
  else if(t.type==="{ORB_TYPE}"){{drawTomichiteOrb9e(ctx,t.radius,ts-t.born,t.lifetime);}}
...
with open(SRC, "w") as f:
    f.write(src)
print(f"Batch 813 done! +{len(src)-original_len} bytes")
