...
FOX_COLOR = "#64748b"; FOX_GLOW = "#f1f5f9"; FOX_PEAK = "🌙"
ORB_COLOR = "#0ea5e9"; ORB_GLOW = "#e0f2fe"; ORB_PEAK = "🦋"
FOX_SC   = 636;  ORB_SC  = 631
FOX_R    = "BASE_R*2.71"; ORB_R = "BASE_R*2.70"
OSC      = 0.4553; SW = 2446
PW1 = "BLAZE2_GLOW44"; PW1_SC = 2768; PW1_SHOCK = "#ef4444"; PW1_ICON = "🔥💫"
PW2 = "FLARE2_GLOW44"; PW2_SC = 2770; PW2_SHOCK = "#dc2626"; PW2_ICON = "🌋✨"

# Prev-batch anchors (batch 825)
PREV_FOX_FULL = "anthoinite_fox9e"
PREV_FOX_COLOR = "#fb923c"; PREV_FOX_GLOW = "#fff7ed"
PREV_PW1  = "FLUX2_GLOW44"; PREV_PW1_ICON = "⚡💫"
PREV_DRAW_FOX = "drawAnthoiniteFox9e"
PREV_FOX_R = "BASE_R*2.70"

# ── Step 1 – 8 achievements ───────────────────────────────────────────────────
ACH = f'''  {{ id:"{FOX_TYPE}_tap",          label:"Aramayoite Fox Tap",          desc:"Tap Aramayoite Fox target",             icon:"{FOX_PEAK}", xp:62 }},
  {{ id:"{FOX_TYPE}_peak",         label:"Aramayoite Fox Peak",         desc:"Reach peak with Aramayoite Fox",        icon:"{FOX_PEAK}", xp:124 }},
  {{ id:"{ORB_TYPE}_tap",          label:"Ardealite Orb Tap",           desc:"Tap Ardealite Orb target",              icon:"{ORB_PEAK}", xp:62 }},
  {{ id:"{ORB_TYPE}_peak",         label:"Ardealite Orb Peak",          desc:"Reach peak with Ardealite Orb",         icon:"{ORB_PEAK}", xp:124 }},
  {{ id:"{PW1.lower()}_use",       label:"Blaze2 Glow",                 desc:"Trigger BLAZE2_GLOW44 power-up",        icon:"🔥", xp:62 }},
  {{ id:"{PW1.lower()}_max",       label:"Blaze2 Glow Max",             desc:"Trigger BLAZE2_GLOW44 at max streak",   icon:"🔥", xp:124 }},
  {{ id:"{PW2.lower()}_use",       label:"Flare2 Glow",                 desc:"Trigger FLARE2_GLOW44 power-up",        icon:"🌋", xp:62 }},
  {{ id:"{PW2.lower()}_max",       label:"Flare2 Glow Max",             desc:"Trigger FLARE2_GLOW44 at max streak",   icon:"🌋", xp:124 }},
...
CMT = f'''// {PW1} — +{PW1_SC} blaze2 glow bonus
    // {PW2} — +{PW2_SC} flare2 glow bonus
    // {PREV_PW1}'''
...
DRAWS = f'''function drawAramayoiteFox9e(ctx,r,ts,sp){{
  const p=ts/sp,pu=p<0.5?p*2:2-p*2;
  ctx.save();
  ctx.shadowColor="{FOX_GLOW}";ctx.shadowBlur=20+pu*18;
  ctx.beginPath();ctx.arc(0,0,r,0,Math.PI*2);
  const g=ctx.createRadialGradient(0,0,r*0.1,0,0,r);
  g.addColorStop(0,"#cbd5e1");g.addColorStop(0.5,"{FOX_COLOR}");g.addColorStop(1,"#1e293b");
...
function drawArdealiteOrb9e(ctx,r,ts,tp){{
  const p=ts/tp,pu=p<0.5?p*2:2-p*2;
  ctx.save();
  ctx.shadowColor="{ORB_GLOW}";ctx.shadowBlur=18+pu*16;
  ctx.beginPath();ctx.arc(0,0,r,0,Math.PI*2);
  const g=ctx.createRadialGradient(0,0,r*0.1,0,0,r);
  g.addColorStop(0,"#bae6fd");g.addColorStop(0.5,"{ORB_COLOR}");g.addColorStop(1,"#0c4a6e");
  ctx.fillStyle=g;ctx.fill();
...
DISP = f'''  else if(t.type==="{FOX_TYPE}"){{drawAramayoiteFox9e(ctx,t.radius,ts-t.born,t.lifetime);}}
  else if(t.type==="{ORB_TYPE}"){{drawArdealiteOrb9e(ctx,t.radius,ts-t.born,t.lifetime);}}
...
with open(SRC, "w") as f:
    f.write(src)
print(f"Batch 826 done! +{len(src)-original_len} bytes")
