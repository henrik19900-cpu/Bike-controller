...
FOX_COLOR = "#f59e0b"; FOX_GLOW = "#fffbeb"; FOX_PEAK = "✨"
ORB_COLOR = "#7c3aed"; ORB_GLOW = "#ede9fe"; ORB_PEAK = "💜"
FOX_SC   = 606;  ORB_SC  = 601
FOX_R    = "BASE_R*2.56"; ORB_R = "BASE_R*2.55"
OSC      = 0.4493; SW = 2416
PW1 = "GROVE_GLOW44";  PW1_SC = 2708; PW1_SHOCK = "#166534"; PW1_ICON = "🌲💫"
PW2 = "MOSS_GLOW44";   PW2_SC = 2710; PW2_SHOCK = "#14532d"; PW2_ICON = "🌱✨"

# Prev-batch anchors (batch 810)
PREV_FOX_FULL = "strunzite_fox9e"
PREV_FOX_COLOR = "#b45309"; PREV_FOX_GLOW = "#fef3c7"
PREV_PW1  = "BARK_GLOW44"; PREV_PW1_ICON = "🪵💫"
PREV_DRAW_FOX = "drawStrunziteFox9e"
PREV_FOX_R = "BASE_R*2.55"

# ── Step 1 – 8 achievements ───────────────────────────────────────────────────
ACH = f'''  {{ id:"{FOX_TYPE}_tap",          label:"Tetradymite Fox Tap",         desc:"Tap Tetradymite Fox target",            icon:"{FOX_PEAK}", xp:62 }},
  {{ id:"{FOX_TYPE}_peak",         label:"Tetradymite Fox Peak",        desc:"Reach peak with Tetradymite Fox",       icon:"{FOX_PEAK}", xp:124 }},
  {{ id:"{ORB_TYPE}_tap",          label:"Tiemannite Orb Tap",          desc:"Tap Tiemannite Orb target",             icon:"{ORB_PEAK}", xp:62 }},
  {{ id:"{ORB_TYPE}_peak",         label:"Tiemannite Orb Peak",         desc:"Reach peak with Tiemannite Orb",        icon:"{ORB_PEAK}", xp:124 }},
  {{ id:"{PW1.lower()}_use",       label:"Grove Glow",                  desc:"Trigger GROVE_GLOW44 power-up",         icon:"🌲", xp:62 }},
  {{ id:"{PW1.lower()}_max",       label:"Grove Glow Max",              desc:"Trigger GROVE_GLOW44 at max streak",    icon:"🌲", xp:124 }},
  {{ id:"{PW2.lower()}_use",       label:"Moss Glow",                   desc:"Trigger MOSS_GLOW44 power-up",          icon:"🌱", xp:62 }},
  {{ id:"{PW2.lower()}_max",       label:"Moss Glow Max",               desc:"Trigger MOSS_GLOW44 at max streak",     icon:"🌱", xp:124 }},
...
CMT = f'''// {PW1} — +{PW1_SC} grove glow bonus
    // {PW2} — +{PW2_SC} moss glow bonus
    // {PREV_PW1}'''
...
DRAWS = f'''function drawTetradymiteFox9e(ctx,r,ts,sp){{
  const p=ts/sp,pu=p<0.5?p*2:2-p*2;
  ctx.save();
  ctx.shadowColor="{FOX_GLOW}";ctx.shadowBlur=20+pu*18;
  ctx.beginPath();ctx.arc(0,0,r,0,Math.PI*2);
  const g=ctx.createRadialGradient(0,0,r*0.1,0,0,r);
  g.addColorStop(0,"#fde68a");g.addColorStop(0.5,"{FOX_COLOR}");g.addColorStop(1,"#92400e");
...
function drawTiemanniteOrb9e(ctx,r,ts,tp){{
  const p=ts/tp,pu=p<0.5?p*2:2-p*2;
  ctx.save();
  ctx.shadowColor="{ORB_GLOW}";ctx.shadowBlur=18+pu*16;
  ctx.beginPath();ctx.arc(0,0,r,0,Math.PI*2);
  const g=ctx.createRadialGradient(0,0,r*0.1,0,0,r);
  g.addColorStop(0,"#c4b5fd");g.addColorStop(0.5,"{ORB_COLOR}");g.addColorStop(1,"#4c1d95");
  ctx.fillStyle=g;ctx.fill();
...
DISP = f'''  else if(t.type==="{FOX_TYPE}"){{drawTetradymiteFox9e(ctx,t.radius,ts-t.born,t.lifetime);}}
  else if(t.type==="{ORB_TYPE}"){{drawTiemanniteOrb9e(ctx,t.radius,ts-t.born,t.lifetime);}}
...
with open(SRC, "w") as f:
    f.write(src)
print(f"Batch 811 done! +{len(src)-original_len} bytes")
