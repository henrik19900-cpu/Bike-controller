...
FOX   = "diadochite_fox9e"
ORB   = "dickinsonite_orb9e"
PW1   = "CLOUD_GLOW44"
PW2   = "FOG_GLOW44"
PW1SC = 2472
PW2SC = 2474
...
OSC   = "0.4257"
SW    = "2298"
FOX_C = "#b45309"; FOX_G = "#fef3c7"
ORB_C = "#0369a1"; ORB_G = "#e0f2fe"
FOX_L = "#fde68a"; FOX_M = "#f59e0b"; FOX_D = "#78350f"
FOX_RL,FOX_RG,FOX_RB = 253,230,138
ORB_L = "#7dd3fc"; ORB_M = "#2563eb"; ORB_D = "#1e3a8a"
ORB_RL,ORB_RG,ORB_RB = 125,211,252
PW1C = "#e2e8f0"; PW2C = "#94a3b8"
PEAK_FOX = "☀️"; PEAK_ORB = "🌀"

# STEP 1
A1_ANCHOR = '{ id:"cumengeite_fox9e_tap", label:"Cumengeite Fox Tap"'
assert A1_ANCHOR in code, "S1 anchor missing"
A1_NEW = (
  '{ id:"diadochite_fox9e_tap", label:"Diadochite Fox Tap", desc:"Tap Diadochite Fox", icon:"🦊", xp:62 },\n'
  '  { id:"diadochite_fox9e_peak", label:"Diadochite Fox Peak", desc:"Hit Diadochite Fox at peak", icon:"☀️", xp:78 },\n'
  '  { id:"dickinsonite_orb9e_tap", label:"Dickinsonite Orb Tap", desc:"Tap Dickinsonite Orb", icon:"🔮", xp:60 },\n'
  '  { id:"dickinsonite_orb9e_peak", label:"Dickinsonite Orb Peak", desc:"Hit Dickinsonite Orb at peak", icon:"🌀", xp:76 },\n'
  '  { id:"cloud_glow44_use", label:"Cloud Glow Use", desc:"Collect Cloud Glow power-up", icon:"☁️", xp:44 },\n'
  '  { id:"cloud_glow44_max", label:"Cloud Glow Max", desc:"Collect 5 Cloud Glow power-ups", icon:"☁️", xp:88 },\n'
  '  { id:"fog_glow44_use", label:"Fog Glow Use", desc:"Collect Fog Glow power-up", icon:"🌫️", xp:44 },\n'
  '  { id:"fog_glow44_max", label:"Fog Glow Max", desc:"Collect 5 Fog Glow power-ups", icon:"🌫️", xp:88 },\n'
  '  { id:"cumengeite_fox9e_tap", label:"Cumengeite Fox Tap"'
)
code = code.replace(A1_ANCHOR, A1_NEW, 1)
assert '{ id:"diadochite_fox9e_tap"' in code, "S1 failed"
print("S1 ok")

# STEP 2
A2_ANCHOR = '"BREEZE_GLOW44","MIST_GLOW44","WAVE_GLOW44"'
assert A2_ANCHOR in code, "S2 anchor missing"
A2_NEW = f'"{PW1}","{PW2}","BREEZE_GLOW44","MIST_GLOW44","WAVE_GLOW44"'
code = code.replace(A2_ANCHOR, A2_NEW, 1)
assert f'"{PW1}"' in code, "S2 failed"
print("S2 ok")

# STEP 3
A3_ANCHOR = '} else if(ptype==="BREEZE_GLOW44"){'
...
# STEP 4
A4_ANCHOR = '// BREEZE_GLOW44 — +2468 breeze glow bonus'
assert A4_ANCHOR in code, "S4 anchor missing"
A4_NEW = (
  f'// {PW1} — +{PW1SC} cloud glow bonus\n'
  f'      // {PW2} — +{PW2SC} fog glow bonus\n'
  f'      // BREEZE_GLOW44 — +2468 breeze glow bonus'
)
...
# STEP 5
A5_ANCHOR = 'function drawCumengeIteFox9e('
assert A5_ANCHOR in code, "S5 anchor missing"
DRAW = (
f'function drawDiadochiteFox9e(ctx,r,ts,sp){{\n'
...
f'  ctx.restore();\n'
f'}}\n'
f'function drawDickinsoniteOrb9e(ctx,r,ts,tp){{\n'
...
f'  ctx.restore();\n'
f'}}\n'
f'function drawCumengeIteFox9e('
)
code = code.replace(A5_ANCHOR, DRAW, 1)
assert 'function drawDiadochiteFox9e(' in code, "S5 failed"
print("S5 ok")

# STEP 6
A6_ANCHOR = '  else if(t.type==="cumengeite_fox9e"){'
...
f'    drawDiadochiteFox9e(ctx,t.radius,ts,sp2);ctx.restore();\n'
f'  }}\n'
f'  else if(t.type==="{ORB}"){{\n'
f'    ctx.save();ctx.translate(t.x,t.y);\n'
f'    const tp2=Math.min(1,(ts-t.born)/{SW});\n'
f'    drawDickinsoniteOrb9e(ctx,t.radius,ts,tp2);ctx.restore();\n'
f'  }}\n'
f'  else if(t.type==="cumengeite_fox9e"){{'
)
...
# STEP 7
A7_ANCHOR = 'if(hit.type==="cumengeite_fox9e"){'
...
f'      if(isPeak)unlock("diadochite_fox9e_peak");\n'
f'      unlock("diadochite_fox9e_tap");\n'
...
f'      addParticle(hit.x,hit.y,"popup",isPeak?"PEAK! +"+pts:"🔮 +"+pts);\n'
f'      if(isPeak)unlock("dickinsonite_orb9e_peak");\n'
f'      unlock("dickinsonite_orb9e_tap");\n'
f'      updateMissions({{...gs.sessionStats,rareHits:gs.sessionStats.rareHits}});\n'
f'      sfx("epic");vibrate([40,10,20]);\n'
f'    }}else if(hit.type==="cumengeite_fox9e"){{'
)
...
# STEP 8
A8_OLD = ('      type="cumengeite_fox9e";color="#4338ca";glow="#eef2ff";\n'
          '    } else if('+COND100F+'){\n'
          '      type="cyrtolite_orb9e"')
...
A8_NEW = (
    ...
    f'    }} else if({COND100F}){{\n'
    f'      type="cumengeite_fox9e";color="#4338ca";glow="#eef2ff";\n'
    f'    }} else if({COND100F}){{\n'
    f'      type="cyrtolite_orb9e"'
)
...
# STEP 9
A9_ANCHOR = 'type==="cumengeite_fox9e"?BASE_R*1.96:type==="cyrtolite_orb9e"?BASE_R*1.95:'
...
# STEP 10
A10_ANCHOR = 'BREEZE_GLOW44:"💨✨",MIST_GLOW44:"🌫️✨",WAVE_GLOW44:"🌊✨"'
cnt = code.count(A10_ANCHOR)
assert cnt == 2, f"S10 anchor count={cnt}, expected 2"
A10_NEW = f'{PW1}:"☁️✨",{PW2}:"🌫️✨",BREEZE_GLOW44:"💨✨",MIST_GLOW44:"🌫️✨",WAVE_GLOW44:"🌊✨"'
...
with open(SRC, "w") as f:
    f.write(code)
print(f"Batch 752 done! +{len(code)-orig_len} bytes")
