...
FOX_COLOR = "#8b5cf6"; FOX_GLOW = "#ede9fe"; FOX_PEAK = "🔮"
ORB_COLOR = "#84cc16"; ORB_GLOW = "#f7fee7"; ORB_PEAK = "☘️"
FOX_SC   = 632;  ORB_SC  = 627
FOX_R    = "BASE_R*2.69"; ORB_R = "BASE_R*2.68"
OSC      = 0.4545; SW = 2442
PW1 = "DRIFT2_GLOW44"; PW1_SC = 2760; PW1_SHOCK = "#7c3aed"; PW1_ICON = "🌊💫"
PW2 = "SURGE2_GLOW44"; PW2_SC = 2762; PW2_SHOCK = "#6d28d9"; PW2_ICON = "⚡✨"

# Prev-batch anchors (batch 823)
PREV_FOX_FULL = "alleghanyite_fox9e"
PREV_FOX_COLOR = "#ec4899"; PREV_FOX_GLOW = "#fdf2f8"
PREV_PW1  = "CREST2_GLOW44"; PREV_PW1_ICON = "🏄💫"
PREV_DRAW_FOX = "drawAlleghanyiteFox9e"
PREV_FOX_R = "BASE_R*2.68"

# ── Step 1 – 8 achievements ───────────────────────────────────────────────────
ACH = f'''  {{ id:"{FOX_TYPE}_tap",          label:"Ammite Fox Tap",              desc:"Tap Ammite Fox target",                 icon:"{FOX_PEAK}", xp:62 }},
  {{ id:"{FOX_TYPE}_peak",         label:"Ammite Fox Peak",             desc:"Reach peak with Ammite Fox",            icon:"{FOX_PEAK}", xp:124 }},
  {{ id:"{ORB_TYPE}_tap",          label:"Andersonite Orb Tap",         desc:"Tap Andersonite Orb target",            icon:"{ORB_PEAK}", xp:62 }},
  {{ id:"{ORB_TYPE}_peak",         label:"Andersonite Orb Peak",        desc:"Reach peak with Andersonite Orb",       icon:"{ORB_PEAK}", xp:124 }},
  {{ id:"{PW1.lower()}_use",       label:"Drift2 Glow",                 desc:"Trigger DRIFT2_GLOW44 power-up",        icon:"🌊", xp:62 }},
  {{ id:"{PW1.lower()}_max",       label:"Drift2 Glow Max",             desc:"Trigger DRIFT2_GLOW44 at max streak",   icon:"🌊", xp:124 }},
  {{ id:"{PW2.lower()}_use",       label:"Surge2 Glow",                 desc:"Trigger SURGE2_GLOW44 power-up",        icon:"⚡", xp:62 }},
  {{ id:"{PW2.lower()}_max",       label:"Surge2 Glow Max",             desc:"Trigger SURGE2_GLOW44 at max streak",   icon:"⚡", xp:124 }},
...
CMT = f'''// {PW1} — +{PW1_SC} drift2 glow bonus
    // {PW2} — +{PW2_SC} surge2 glow bonus
    // {PREV_PW1}'''
...
DRAWS = f'''function drawAmmiteFox9e(ctx,r,ts,sp){{
  const p=ts/sp,pu=p<0.5?p*2:2-p*2;
  ctx.save();
  ctx.shadowColor="{FOX_GLOW}";ctx.shadowBlur=20+pu*18;
  ctx.beginPath();ctx.arc(0,0,r,0,Math.PI*2);
  const g=ctx.createRadialGradient(0,0,r*0.1,0,0,r);
  g.addColorStop(0,"#ddd6fe");g.addColorStop(0.5,"{FOX_COLOR}");g.addColorStop(1,"#4c1d95");
...
function drawAndersoniteOrb9e(ctx,r,ts,tp){{
  const p=ts/tp,pu=p<0.5?p*2:2-p*2;
  ctx.save();
  ctx.shadowColor="{ORB_GLOW}";ctx.shadowBlur=18+pu*16;
  ctx.beginPath();ctx.arc(0,0,r,0,Math.PI*2);
  const g=ctx.createRadialGradient(0,0,r*0.1,0,0,r);
  g.addColorStop(0,"#d9f99d");g.addColorStop(0.5,"{ORB_COLOR}");g.addColorStop(1,"#365314");
  ctx.fillStyle=g;ctx.fill();
...
DISP = f'''  else if(t.type==="{FOX_TYPE}"){{drawAmmiteFox9e(ctx,t.radius,ts-t.born,t.lifetime);}}
  else if(t.type==="{ORB_TYPE}"){{drawAndersoniteOrb9e(ctx,t.radius,ts-t.born,t.lifetime);}}
...
with open(SRC, "w") as f:
    f.write(src)
print(f"Batch 824 done! +{len(src)-original_len} bytes")
