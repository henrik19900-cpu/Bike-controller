...
ORB_C = "#312e81"; ORB_G = "#eef2ff"
FOX_L = "#bae6fd"; FOX_M = "#0369a1"; FOX_D = "#0c4a6e"
FOX_RL,FOX_RG,FOX_RB = 186,230,253
ORB_L = "#c7d2fe"; ORB_M = "#4338ca"; ORB_D = "#1e1b4b"
ORB_RL,ORB_RG,ORB_RB = 199,210,254
PW1C = "#1d4ed8"; PW2C = "#1e1b4b"
PEAK_FOX = "🌊"; PEAK_ORB = "🌌"

# STEP 1
A1_ANCHOR = '{ id:"gudmundite_fox9e_tap", label:"Gudmundite Fox Tap"'
assert A1_ANCHOR in code, "S1 anchor missing"
A1_NEW = (
  '{ id:"gyrolite_fox9e_tap", label:"Gyrolite Fox Tap", desc:"Tap Gyrolite Fox", icon:"🦊", xp:62 },\n'
  '  { id:"gyrolite_fox9e_peak", label:"Gyrolite Fox Peak", desc:"Hit Gyrolite Fox at peak", icon:"🌊", xp:78 },\n'
  '  { id:"hambergite_orb9e_tap", label:"Hambergite Orb Tap", desc:"Tap Hambergite Orb", icon:"🔮", xp:60 },\n'
  '  { id:"hambergite_orb9e_peak", label:"Hambergite Orb Peak", desc:"Hit Hambergite Orb at peak", icon:"🌌", xp:76 },\n'
  '  { id:"deep_glow44_use", label:"Deep Glow Use", desc:"Collect Deep Glow power-up", icon:"🌊", xp:44 },\n'
  '  { id:"deep_glow44_max", label:"Deep Glow Max", desc:"Collect 5 Deep Glow power-ups", icon:"🌊", xp:88 },\n'
  '  { id:"trench_glow44_use", label:"Trench Glow Use", desc:"Collect Trench Glow power-up", icon:"🌌", xp:44 },\n'
  '  { id:"trench_glow44_max", label:"Trench Glow Max", desc:"Collect 5 Trench Glow power-ups", icon:"🌌", xp:88 },\n'
  '  { id:"gudmundite_fox9e_tap", label:"Gudmundite Fox Tap"'
)
code = code.replace(A1_ANCHOR, A1_NEW, 1)
assert '{ id:"gyrolite_fox9e_tap"' in code, "S1 failed"
print("S1 ok")

# STEP 2
A2_ANCHOR = '"CORAL_GLOW44","ABYSS_GLOW44","REEF_GLOW44"'
assert A2_ANCHOR in code, "S2 anchor missing"
A2_NEW = f'"{PW1}","{PW2}","CORAL_GLOW44","ABYSS_GLOW44","REEF_GLOW44"'
code = code.replace(A2_ANCHOR, A2_NEW, 1)
assert f'"{PW1}"' in code, "S2 failed"
print("S2 ok")

# STEP 3
A3_ANCHOR = '} else if(ptype==="CORAL_GLOW44"){'
...
# STEP 4
A4_ANCHOR = '// CORAL_GLOW44 — +2524 coral glow bonus'
assert A4_ANCHOR in code, "S4 anchor missing"
A4_NEW = (
  f'// {PW1} — +{PW1SC} deep glow bonus\n'
  f'      // {PW2} — +{PW2SC} trench glow bonus\n'
  f'      // CORAL_GLOW44 — +2524 coral glow bonus'
)
...
# STEP 5
A5_ANCHOR = 'function drawGudmunditeFox9e('
assert A5_ANCHOR in code, "S5 anchor missing"
DRAW = (
f'function drawGyroliteFox9e(ctx,r,ts,sp){{\n'
...
f'  ctx.restore();\n'
f'}}\n'
f'function drawHambergiteOrb9e(ctx,r,ts,tp){{\n'
...
f'  ctx.restore();\n'
f'}}\n'
f'function drawGudmunditeFox9e('
)
code = code.replace(A5_ANCHOR, DRAW, 1)
assert 'function drawGyroliteFox9e(' in code, "S5 failed"
print("S5 ok")

# STEP 6
A6_ANCHOR = '  else if(t.type==="gudmundite_fox9e"){'
...
f'    drawGyroliteFox9e(ctx,t.radius,ts,sp2);ctx.restore();\n'
f'  }}\n'
f'  else if(t.type==="{ORB}"){{\n'
f'    ctx.save();ctx.translate(t.x,t.y);\n'
f'    const tp2=Math.min(1,(ts-t.born)/{SW});\n'
f'    drawHambergiteOrb9e(ctx,t.radius,ts,tp2);ctx.restore();\n'
f'  }}\n'
f'  else if(t.type==="gudmundite_fox9e"){{'
)
...
# STEP 7
A7_ANCHOR = 'if(hit.type==="gudmundite_fox9e"){'
...
f'      if(isPeak)unlock("gyrolite_fox9e_peak");\n'
f'      unlock("gyrolite_fox9e_tap");\n'
...
f'      addParticle(hit.x,hit.y,"popup",isPeak?"PEAK! +"+pts:"🔮 +"+pts);\n'
f'      if(isPeak)unlock("hambergite_orb9e_peak");\n'
f'      unlock("hambergite_orb9e_tap");\n'
f'      updateMissions({{...gs.sessionStats,rareHits:gs.sessionStats.rareHits}});\n'
f'      sfx("epic");vibrate([40,10,20]);\n'
f'    }}else if(hit.type==="gudmundite_fox9e"){{'
)
...
# STEP 8
A8_OLD = ('      type="gudmundite_fox9e";color="#be185d";glow="#fff1f2";\n'
          '    } else if('+COND100F+'){\n'
          '      type="gummite_orb9e"')
...
A8_NEW = (
    ...
    f'    }} else if({COND100F}){{\n'
    f'      type="gudmundite_fox9e";color="#be185d";glow="#fff1f2";\n'
    f'    }} else if({COND100F}){{\n'
    f'      type="gummite_orb9e"'
)
...
# STEP 9
A9_ANCHOR = 'type==="gudmundite_fox9e"?BASE_R*2.10:type==="gummite_orb9e"?BASE_R*2.09:'
...
# STEP 10
A10_ANCHOR = 'CORAL_GLOW44:"🪸✨",ABYSS_GLOW44:"🌑✨",REEF_GLOW44:"🐠✨"'
cnt = code.count(A10_ANCHOR)
assert cnt == 2, f"S10 anchor count={cnt}, expected 2"
A10_NEW = f'{PW1}:"🌊✨",{PW2}:"🌌✨",CORAL_GLOW44:"🪸✨",ABYSS_GLOW44:"🌑✨",REEF_GLOW44:"🐠✨"'
...
with open(SRC, "w") as f:
    f.write(code)
print(f"Batch 766 done! +{len(code)-orig_len} bytes")
