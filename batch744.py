...
OSC   = "0.4225"
SW    = "2282"
FOX_C = "#0369a1"; FOX_G = "#e0f2fe"
ORB_C = "#047857"; ORB_G = "#ecfdf5"
FOX_L = "#7dd3fc"; FOX_M = "#0284c7"; FOX_D = "#082f49"
FOX_RL,FOX_RG,FOX_RB = 125,211,252
ORB_L = "#6ee7b7"; ORB_M = "#059669"; ORB_D = "#064e3b"
ORB_RL,ORB_RG,ORB_RB = 110,231,183
PW1C = "#fbbf24"; PW2C = "#34d399"
PEAK_FOX = "💙"; PEAK_ORB = "💚"

# STEP 1
A1_ANCHOR = '{ id:"carnotite_fox9e_tap", label:"Carnotite Fox Tap"'
assert A1_ANCHOR in code, "S1 anchor missing"
A1_NEW = (
  '{ id:"celestine_fox9e_tap", label:"Celestine Fox Tap", desc:"Tap Celestine Fox", icon:"🦊", xp:62 },\n'
  '  { id:"celestine_fox9e_peak", label:"Celestine Fox Peak", desc:"Hit Celestine Fox at peak", icon:"💙", xp:78 },\n'
  '  { id:"chabazite_orb9e_tap", label:"Chabazite Orb Tap", desc:"Tap Chabazite Orb", icon:"🔮", xp:60 },\n'
  '  { id:"chabazite_orb9e_peak", label:"Chabazite Orb Peak", desc:"Hit Chabazite Orb at peak", icon:"💚", xp:76 },\n'
  '  { id:"gleam_glow44_use", label:"Gleam Glow Use", desc:"Collect Gleam Glow power-up", icon:"✨", xp:44 },\n'
  '  { id:"gleam_glow44_max", label:"Gleam Glow Max", desc:"Collect 5 Gleam Glow power-ups", icon:"✨", xp:88 },\n'
  '  { id:"glint_glow44_use", label:"Glint Glow Use", desc:"Collect Glint Glow power-up", icon:"💎", xp:44 },\n'
  '  { id:"glint_glow44_max", label:"Glint Glow Max", desc:"Collect 5 Glint Glow power-ups", icon:"💎", xp:88 },\n'
  '  { id:"carnotite_fox9e_tap", label:"Carnotite Fox Tap"'
)
code = code.replace(A1_ANCHOR, A1_NEW, 1)
assert '{ id:"celestine_fox9e_tap"' in code, "S1 failed"
print("S1 ok")

# STEP 2
A2_ANCHOR = '"FLARE_GLOW44","BLAZE_GLOW44","SHADOW_GLOW44"'
assert A2_ANCHOR in code, "S2 anchor missing"
A2_NEW = f'"{PW1}","{PW2}","FLARE_GLOW44","BLAZE_GLOW44","SHADOW_GLOW44"'
code = code.replace(A2_ANCHOR, A2_NEW, 1)
assert f'"{PW1}"' in code, "S2 failed"
print("S2 ok")

# STEP 3
A3_ANCHOR = '} else if(ptype==="FLARE_GLOW44"){'
...
# STEP 4
A4_ANCHOR = '// FLARE_GLOW44 — +2436 flare glow bonus'
assert A4_ANCHOR in code, "S4 anchor missing"
A4_NEW = (
  f'// {PW1} — +{PW1SC} gleam glow bonus\n'
  f'      // {PW2} — +{PW2SC} glint glow bonus\n'
  f'      // FLARE_GLOW44 — +2436 flare glow bonus'
)
...
# STEP 5
A5_ANCHOR = 'function drawCarnotiteFox9e('
assert A5_ANCHOR in code, "S5 anchor missing"
DRAW = (
f'function drawCelestineFox9e(ctx,r,ts,sp){{\n'
...
f'  ctx.restore();\n'
f'}}\n'
f'function drawChabaziteOrb9e(ctx,r,ts,tp){{\n'
...
f'  ctx.restore();\n'
f'}}\n'
f'function drawCarnotiteFox9e('
)
code = code.replace(A5_ANCHOR, DRAW, 1)
assert 'function drawCelestineFox9e(' in code, "S5 failed"
print("S5 ok")

# STEP 6
A6_ANCHOR = '  else if(t.type==="carnotite_fox9e"){'
...
f'    drawCelestineFox9e(ctx,t.radius,ts,sp2);ctx.restore();\n'
f'  }}\n'
f'  else if(t.type==="{ORB}"){{\n'
f'    ctx.save();ctx.translate(t.x,t.y);\n'
f'    const tp2=Math.min(1,(ts-t.born)/{SW});\n'
f'    drawChabaziteOrb9e(ctx,t.radius,ts,tp2);ctx.restore();\n'
f'  }}\n'
f'  else if(t.type==="carnotite_fox9e"){{'
)
...
# STEP 7
A7_ANCHOR = 'if(hit.type==="carnotite_fox9e"){'
...
f'      if(isPeak)unlock("celestine_fox9e_peak");\n'
f'      unlock("celestine_fox9e_tap");\n'
...
f'      addParticle(hit.x,hit.y,"popup",isPeak?"PEAK! +"+pts:"🔮 +"+pts);\n'
f'      if(isPeak)unlock("chabazite_orb9e_peak");\n'
f'      unlock("chabazite_orb9e_tap");\n'
f'      updateMissions({{...gs.sessionStats,rareHits:gs.sessionStats.rareHits}});\n'
f'      sfx("epic");vibrate([40,10,20]);\n'
f'    }}else if(hit.type==="carnotite_fox9e"){{'
)
...
# STEP 8
A8_OLD = ('      type="carnotite_fox9e";color="#d97706";glow="#fffbeb";\n'
          '    } else if('+COND100F+'){\n'
          '      type="catapleiite_orb9e"')
...
A8_NEW = (
    ...
    f'    }} else if({COND100F}){{\n'
    f'      type="carnotite_fox9e";color="#d97706";glow="#fffbeb";\n'
    f'    }} else if({COND100F}){{\n'
    f'      type="catapleiite_orb9e"'
)
...
# STEP 9
A9_ANCHOR = 'type==="carnotite_fox9e"?BASE_R*1.88:type==="catapleiite_orb9e"?BASE_R*1.87:'
...
# STEP 10
A10_ANCHOR = 'FLARE_GLOW44:"🔥✨",BLAZE_GLOW44:"🔥✨",SHADOW_GLOW44:"🌑✨"'
cnt = code.count(A10_ANCHOR)
assert cnt == 2, f"S10 anchor count={cnt}, expected 2"
A10_NEW = f'{PW1}:"✨✨",{PW2}:"💎✨",FLARE_GLOW44:"🔥✨",BLAZE_GLOW44:"🔥✨",SHADOW_GLOW44:"🌑✨"'
...
with open(SRC, "w") as f:
    f.write(code)
print(f"Batch 744 done! +{len(code)-orig_len} bytes")
