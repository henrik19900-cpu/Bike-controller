...
OSC   = "0.4261"
SW    = "2300"
FOX_C = "#166534"; FOX_G = "#f0fdf4"
ORB_C = "#1e3a8a"; ORB_G = "#eff6ff"
FOX_L = "#bbf7d0"; FOX_M = "#22c55e"; FOX_D = "#052e16"
FOX_RL,FOX_RG,FOX_RB = 187,247,208
ORB_L = "#bfdbfe"; ORB_M = "#1d4ed8"; ORB_D = "#1e1b4b"
ORB_RL,ORB_RG,ORB_RB = 191,219,254
PW1C = "#a78bfa"; PW2C = "#60a5fa"
PEAK_FOX = "🌿"; PEAK_ORB = "🌧️"

# STEP 1
A1_ANCHOR = '{ id:"diadochite_fox9e_tap", label:"Diadochite Fox Tap"'
assert A1_ANCHOR in code, "S1 anchor missing"
A1_NEW = (
  '{ id:"dioptase_fox9e_tap", label:"Dioptase Fox Tap", desc:"Tap Dioptase Fox", icon:"🦊", xp:62 },\n'
  '  { id:"dioptase_fox9e_peak", label:"Dioptase Fox Peak", desc:"Hit Dioptase Fox at peak", icon:"🌿", xp:78 },\n'
  '  { id:"domeykite_orb9e_tap", label:"Domeykite Orb Tap", desc:"Tap Domeykite Orb", icon:"🔮", xp:60 },\n'
  '  { id:"domeykite_orb9e_peak", label:"Domeykite Orb Peak", desc:"Hit Domeykite Orb at peak", icon:"🌧️", xp:76 },\n'
  '  { id:"haze_glow44_use", label:"Haze Glow Use", desc:"Collect Haze Glow power-up", icon:"🌫️", xp:44 },\n'
  '  { id:"haze_glow44_max", label:"Haze Glow Max", desc:"Collect 5 Haze Glow power-ups", icon:"🌫️", xp:88 },\n'
  '  { id:"rain_glow44_use", label:"Rain Glow Use", desc:"Collect Rain Glow power-up", icon:"🌧️", xp:44 },\n'
  '  { id:"rain_glow44_max", label:"Rain Glow Max", desc:"Collect 5 Rain Glow power-ups", icon:"🌧️", xp:88 },\n'
  '  { id:"diadochite_fox9e_tap", label:"Diadochite Fox Tap"'
)
code = code.replace(A1_ANCHOR, A1_NEW, 1)
assert '{ id:"dioptase_fox9e_tap"' in code, "S1 failed"
print("S1 ok")

# STEP 2
A2_ANCHOR = '"CLOUD_GLOW44","FOG_GLOW44","BREEZE_GLOW44"'
assert A2_ANCHOR in code, "S2 anchor missing"
A2_NEW = f'"{PW1}","{PW2}","CLOUD_GLOW44","FOG_GLOW44","BREEZE_GLOW44"'
code = code.replace(A2_ANCHOR, A2_NEW, 1)
assert f'"{PW1}"' in code, "S2 failed"
print("S2 ok")

# STEP 3
A3_ANCHOR = '} else if(ptype==="CLOUD_GLOW44"){'
...
# STEP 4
A4_ANCHOR = '// CLOUD_GLOW44 — +2472 cloud glow bonus'
assert A4_ANCHOR in code, "S4 anchor missing"
A4_NEW = (
  f'// {PW1} — +{PW1SC} haze glow bonus\n'
  f'      // {PW2} — +{PW2SC} rain glow bonus\n'
  f'      // CLOUD_GLOW44 — +2472 cloud glow bonus'
)
...
# STEP 5
A5_ANCHOR = 'function drawDiadochiteFox9e('
assert A5_ANCHOR in code, "S5 anchor missing"
DRAW = (
f'function drawDioptaseFox9e(ctx,r,ts,sp){{\n'
...
f'  ctx.restore();\n'
f'}}\n'
f'function drawDomeykiteOrb9e(ctx,r,ts,tp){{\n'
...
f'  ctx.restore();\n'
f'}}\n'
f'function drawDiadochiteFox9e('
)
code = code.replace(A5_ANCHOR, DRAW, 1)
assert 'function drawDioptaseFox9e(' in code, "S5 failed"
print("S5 ok")

# STEP 6
A6_ANCHOR = '  else if(t.type==="diadochite_fox9e"){'
...
f'    drawDioptaseFox9e(ctx,t.radius,ts,sp2);ctx.restore();\n'
f'  }}\n'
f'  else if(t.type==="{ORB}"){{\n'
f'    ctx.save();ctx.translate(t.x,t.y);\n'
f'    const tp2=Math.min(1,(ts-t.born)/{SW});\n'
f'    drawDomeykiteOrb9e(ctx,t.radius,ts,tp2);ctx.restore();\n'
f'  }}\n'
f'  else if(t.type==="diadochite_fox9e"){{'
)
...
# STEP 7
A7_ANCHOR = 'if(hit.type==="diadochite_fox9e"){'
...
f'      if(isPeak)unlock("dioptase_fox9e_peak");\n'
f'      unlock("dioptase_fox9e_tap");\n'
...
f'      addParticle(hit.x,hit.y,"popup",isPeak?"PEAK! +"+pts:"🔮 +"+pts);\n'
f'      if(isPeak)unlock("domeykite_orb9e_peak");\n'
f'      unlock("domeykite_orb9e_tap");\n'
f'      updateMissions({{...gs.sessionStats,rareHits:gs.sessionStats.rareHits}});\n'
f'      sfx("epic");vibrate([40,10,20]);\n'
f'    }}else if(hit.type==="diadochite_fox9e"){{'
)
...
# STEP 8
A8_OLD = ('      type="diadochite_fox9e";color="#b45309";glow="#fef3c7";\n'
          '    } else if('+COND100F+'){\n'
          '      type="dickinsonite_orb9e"')
...
A8_NEW = (
    ...
    f'    }} else if({COND100F}){{\n'
    f'      type="diadochite_fox9e";color="#b45309";glow="#fef3c7";\n'
    f'    }} else if({COND100F}){{\n'
    f'      type="dickinsonite_orb9e"'
)
...
# STEP 9
A9_ANCHOR = 'type==="diadochite_fox9e"?BASE_R*1.97:type==="dickinsonite_orb9e"?BASE_R*1.96:'
...
# STEP 10
A10_ANCHOR = 'CLOUD_GLOW44:"☁️✨",FOG_GLOW44:"🌫️✨",BREEZE_GLOW44:"💨✨"'
cnt = code.count(A10_ANCHOR)
assert cnt == 2, f"S10 anchor count={cnt}, expected 2"
A10_NEW = f'{PW1}:"🌫️✨",{PW2}:"🌧️✨",CLOUD_GLOW44:"☁️✨",FOG_GLOW44:"🌫️✨",BREEZE_GLOW44:"💨✨"'
...
with open(SRC, "w") as f:
    f.write(code)
print(f"Batch 753 done! +{len(code)-orig_len} bytes")
