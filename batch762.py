...
FOX   = "glauberite_fox9e"
ORB   = "glaukosphaerite_orb9e"
PW1   = "OASIS_GLOW44"
PW2   = "BASIN_GLOW44"
PW1SC = 2512
PW2SC = 2514
...
ORB_C = "#0e7490"; ORB_G = "#ecfeff"
FOX_L = "#c7d2fe"; FOX_M = "#6366f1"; FOX_D = "#1e1b4b"
FOX_RL,FOX_RG,FOX_RB = 199,210,254
ORB_L = "#67e8f9"; ORB_M = "#0891b2"; ORB_D = "#083344"
ORB_RL,ORB_RG,ORB_RB = 103,232,249
PW1C = "#34d399"; PW2C = "#06b6d4"
PEAK_FOX = "💫"; PEAK_ORB = "🌊"

# STEP 1
A1_ANCHOR = '{ id:"galenite_fox9e_tap", label:"Galenite Fox Tap"'
assert A1_ANCHOR in code, "S1 anchor missing"
A1_NEW = (
  '{ id:"glauberite_fox9e_tap", label:"Glauberite Fox Tap", desc:"Tap Glauberite Fox", icon:"🦊", xp:62 },\n'
  '  { id:"glauberite_fox9e_peak", label:"Glauberite Fox Peak", desc:"Hit Glauberite Fox at peak", icon:"💫", xp:78 },\n'
  '  { id:"glaukosphaerite_orb9e_tap", label:"Glaukosphaerite Orb Tap", desc:"Tap Glaukosphaerite Orb", icon:"🔮", xp:60 },\n'
  '  { id:"glaukosphaerite_orb9e_peak", label:"Glaukosphaerite Orb Peak", desc:"Hit Glaukosphaerite Orb at peak", icon:"🌊", xp:76 },\n'
  '  { id:"oasis_glow44_use", label:"Oasis Glow Use", desc:"Collect Oasis Glow power-up", icon:"🌴", xp:44 },\n'
  '  { id:"oasis_glow44_max", label:"Oasis Glow Max", desc:"Collect 5 Oasis Glow power-ups", icon:"🌴", xp:88 },\n'
  '  { id:"basin_glow44_use", label:"Basin Glow Use", desc:"Collect Basin Glow power-up", icon:"💧", xp:44 },\n'
  '  { id:"basin_glow44_max", label:"Basin Glow Max", desc:"Collect 5 Basin Glow power-ups", icon:"💧", xp:88 },\n'
  '  { id:"galenite_fox9e_tap", label:"Galenite Fox Tap"'
)
code = code.replace(A1_ANCHOR, A1_NEW, 1)
assert '{ id:"glauberite_fox9e_tap"' in code, "S1 failed"
print("S1 ok")

# STEP 2
A2_ANCHOR = '"CANYON_GLOW44","DUNE_GLOW44","DESERT_GLOW44"'
assert A2_ANCHOR in code, "S2 anchor missing"
A2_NEW = f'"{PW1}","{PW2}","CANYON_GLOW44","DUNE_GLOW44","DESERT_GLOW44"'
code = code.replace(A2_ANCHOR, A2_NEW, 1)
assert f'"{PW1}"' in code, "S2 failed"
print("S2 ok")

# STEP 3
A3_ANCHOR = '} else if(ptype==="CANYON_GLOW44"){'
...
# STEP 4
A4_ANCHOR = '// CANYON_GLOW44 — +2508 canyon glow bonus'
assert A4_ANCHOR in code, "S4 anchor missing"
A4_NEW = (
  f'// {PW1} — +{PW1SC} oasis glow bonus\n'
  f'      // {PW2} — +{PW2SC} basin glow bonus\n'
  f'      // CANYON_GLOW44 — +2508 canyon glow bonus'
)
...
# STEP 5
A5_ANCHOR = 'function drawGaleniteFox9e('
assert A5_ANCHOR in code, "S5 anchor missing"
DRAW = (
f'function drawGlauberiteFox9e(ctx,r,ts,sp){{\n'
...
f'  ctx.restore();\n'
f'}}\n'
f'function drawGlaukosphaeriteOrb9e(ctx,r,ts,tp){{\n'
...
f'  ctx.restore();\n'
f'}}\n'
f'function drawGaleniteFox9e('
)
code = code.replace(A5_ANCHOR, DRAW, 1)
assert 'function drawGlauberiteFox9e(' in code, "S5 failed"
print("S5 ok")

# STEP 6
A6_ANCHOR = '  else if(t.type==="galenite_fox9e"){'
...
f'    drawGlauberiteFox9e(ctx,t.radius,ts,sp2);ctx.restore();\n'
f'  }}\n'
f'  else if(t.type==="{ORB}"){{\n'
f'    ctx.save();ctx.translate(t.x,t.y);\n'
f'    const tp2=Math.min(1,(ts-t.born)/{SW});\n'
f'    drawGlaukosphaeriteOrb9e(ctx,t.radius,ts,tp2);ctx.restore();\n'
f'  }}\n'
f'  else if(t.type==="galenite_fox9e"){{'
)
...
# STEP 7
A7_ANCHOR = 'if(hit.type==="galenite_fox9e"){'
...
f'      if(isPeak)unlock("glauberite_fox9e_peak");\n'
f'      unlock("glauberite_fox9e_tap");\n'
...
f'      addParticle(hit.x,hit.y,"popup",isPeak?"PEAK! +"+pts:"🔮 +"+pts);\n'
f'      if(isPeak)unlock("glaukosphaerite_orb9e_peak");\n'
f'      unlock("glaukosphaerite_orb9e_tap");\n'
f'      updateMissions({{...gs.sessionStats,rareHits:gs.sessionStats.rareHits}});\n'
f'      sfx("epic");vibrate([40,10,20]);\n'
f'    }}else if(hit.type==="galenite_fox9e"){{'
)
...
# STEP 8
A8_OLD = ('      type="galenite_fox9e";color="#78350f";glow="#fef3c7";\n'
          '    } else if('+COND100F+'){\n'
          '      type="gaspeite_orb9e"')
...
A8_NEW = (
    ...
    f'    }} else if({COND100F}){{\n'
    f'      type="galenite_fox9e";color="#78350f";glow="#fef3c7";\n'
    f'    }} else if({COND100F}){{\n'
    f'      type="gaspeite_orb9e"'
)
...
# STEP 9
A9_ANCHOR = 'type==="galenite_fox9e"?BASE_R*2.06:type==="gaspeite_orb9e"?BASE_R*2.05:'
...
# STEP 10
A10_ANCHOR = 'CANYON_GLOW44:"🏜️✨",DUNE_GLOW44:"🏖️✨",DESERT_GLOW44:"🏜️✨"'
cnt = code.count(A10_ANCHOR)
assert cnt == 2, f"S10 anchor count={cnt}, expected 2"
A10_NEW = f'{PW1}:"🌴✨",{PW2}:"💧✨",CANYON_GLOW44:"🏜️✨",DUNE_GLOW44:"🏖️✨",DESERT_GLOW44:"🏜️✨"'
...
with open(SRC, "w") as f:
    f.write(code)
print(f"Batch 762 done! +{len(code)-orig_len} bytes")
