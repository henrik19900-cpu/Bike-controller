...
ORB_C = "#be185d"; ORB_G = "#fdf2f8"
FOX_L = "#fde68a"; FOX_M = "#f59e0b"; FOX_D = "#78350f"
FOX_RL,FOX_RG,FOX_RB = 253,230,138
ORB_L = "#f9a8d4"; ORB_M = "#db2777"; ORB_D = "#500724"
ORB_RL,ORB_RG,ORB_RB = 249,168,212
PW1C = "#f97316"; PW2C = "#ef4444"
PEAK_FOX = "🌟"; PEAK_ORB = "💗"

# STEP 1
A1_ANCHOR = '{ id:"calomel_fox9e_tap", label:"Calomel Fox Tap"'
assert A1_ANCHOR in code, "S1 anchor missing"
A1_NEW = (
  '{ id:"carnotite_fox9e_tap", label:"Carnotite Fox Tap", desc:"Tap Carnotite Fox", icon:"🦊", xp:62 },\n'
  '  { id:"carnotite_fox9e_peak", label:"Carnotite Fox Peak", desc:"Hit Carnotite Fox at peak", icon:"🌟", xp:78 },\n'
  '  { id:"catapleiite_orb9e_tap", label:"Catapleiite Orb Tap", desc:"Tap Catapleiite Orb", icon:"🔮", xp:60 },\n'
  '  { id:"catapleiite_orb9e_peak", label:"Catapleiite Orb Peak", desc:"Hit Catapleiite Orb at peak", icon:"💗", xp:76 },\n'
  '  { id:"flare_glow44_use", label:"Flare Glow Use", desc:"Collect Flare Glow power-up", icon:"🔥", xp:44 },\n'
  '  { id:"flare_glow44_max", label:"Flare Glow Max", desc:"Collect 5 Flare Glow power-ups", icon:"🔥", xp:88 },\n'
  '  { id:"blaze_glow44_use", label:"Blaze Glow Use", desc:"Collect Blaze Glow power-up", icon:"🔥", xp:44 },\n'
  '  { id:"blaze_glow44_max", label:"Blaze Glow Max", desc:"Collect 5 Blaze Glow power-ups", icon:"🔥", xp:88 },\n'
  '  { id:"calomel_fox9e_tap", label:"Calomel Fox Tap"'
)
code = code.replace(A1_ANCHOR, A1_NEW, 1)
assert '{ id:"carnotite_fox9e_tap"' in code, "S1 failed"
print("S1 ok")

# STEP 2
A2_ANCHOR = '"SHADOW_GLOW44","AETHER_GLOW44","ARCANE_GLOW44"'
assert A2_ANCHOR in code, "S2 anchor missing"
A2_NEW = f'"{PW1}","{PW2}","SHADOW_GLOW44","AETHER_GLOW44","ARCANE_GLOW44"'
code = code.replace(A2_ANCHOR, A2_NEW, 1)
assert f'"{PW1}"' in code, "S2 failed"
print("S2 ok")

# STEP 3
A3_ANCHOR = '} else if(ptype==="SHADOW_GLOW44"){'
...
# STEP 4
A4_ANCHOR = '// SHADOW_GLOW44 — +2432 shadow glow bonus'
assert A4_ANCHOR in code, "S4 anchor missing"
A4_NEW = (
  f'// {PW1} — +{PW1SC} flare glow bonus\n'
  f'      // {PW2} — +{PW2SC} blaze glow bonus\n'
  f'      // SHADOW_GLOW44 — +2432 shadow glow bonus'
)
...
# STEP 5
A5_ANCHOR = 'function drawCalomelFox9e('
assert A5_ANCHOR in code, "S5 anchor missing"
DRAW = (
f'function drawCarnotiteFox9e(ctx,r,ts,sp){{\n'
...
f'  ctx.restore();\n'
f'}}\n'
f'function drawCatapleiiteOrb9e(ctx,r,ts,tp){{\n'
...
f'  ctx.restore();\n'
f'}}\n'
f'function drawCalomelFox9e('
)
code = code.replace(A5_ANCHOR, DRAW, 1)
assert 'function drawCarnotiteFox9e(' in code, "S5 failed"
print("S5 ok")

# STEP 6
A6_ANCHOR = '  else if(t.type==="calomel_fox9e"){'
...
f'    drawCarnotiteFox9e(ctx,t.radius,ts,sp2);ctx.restore();\n'
f'  }}\n'
f'  else if(t.type==="{ORB}"){{\n'
f'    ctx.save();ctx.translate(t.x,t.y);\n'
f'    const tp2=Math.min(1,(ts-t.born)/{SW});\n'
f'    drawCatapleiiteOrb9e(ctx,t.radius,ts,tp2);ctx.restore();\n'
f'  }}\n'
f'  else if(t.type==="calomel_fox9e"){{'
)
...
# STEP 7
A7_ANCHOR = 'if(hit.type==="calomel_fox9e"){'
...
f'      if(isPeak)unlock("carnotite_fox9e_peak");\n'
f'      unlock("carnotite_fox9e_tap");\n'
...
f'      addParticle(hit.x,hit.y,"popup",isPeak?"PEAK! +"+pts:"🔮 +"+pts);\n'
f'      if(isPeak)unlock("catapleiite_orb9e_peak");\n'
f'      unlock("catapleiite_orb9e_tap");\n'
f'      updateMissions({{...gs.sessionStats,rareHits:gs.sessionStats.rareHits}});\n'
f'      sfx("epic");vibrate([40,10,20]);\n'
f'    }}else if(hit.type==="calomel_fox9e"){{'
)
...
# STEP 8
A8_OLD = ('      type="calomel_fox9e";color="#6d28d9";glow="#f5f3ff";\n'
          '    } else if('+COND100F+'){\n'
          '      type="carminite_orb9e"')
...
A8_NEW = (
    ...
    f'    }} else if({COND100F}){{\n'
    f'      type="calomel_fox9e";color="#6d28d9";glow="#f5f3ff";\n'
    f'    }} else if({COND100F}){{\n'
    f'      type="carminite_orb9e"'
)
...
# STEP 9
A9_ANCHOR = 'type==="calomel_fox9e"?BASE_R*1.87:type==="carminite_orb9e"?BASE_R*1.86:'
...
# STEP 10
A10_ANCHOR = 'SHADOW_GLOW44:"🌑✨",AETHER_GLOW44:"🌀✨",ARCANE_GLOW44:"🔮✨"'
cnt = code.count(A10_ANCHOR)
assert cnt == 2, f"S10 anchor count={cnt}, expected 2"
A10_NEW = f'{PW1}:"🔥✨",{PW2}:"🔥✨",SHADOW_GLOW44:"🌑✨",AETHER_GLOW44:"🌀✨",ARCANE_GLOW44:"🔮✨"'
...
with open(SRC, "w") as f:
    f.write(code)
print(f"Batch 743 done! +{len(code)-orig_len} bytes")
