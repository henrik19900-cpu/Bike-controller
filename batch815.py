...
FOX_COLOR = "#e2e8f0"; FOX_GLOW = "#f8fafc"; FOX_PEAK = "❄️"
ORB_COLOR = "#dc2626"; ORB_GLOW = "#fee2e2"; ORB_PEAK = "🔴"
FOX_SC   = 614;  ORB_SC  = 609
FOX_R    = "BASE_R*2.60"; ORB_R = "BASE_R*2.59"
OSC      = 0.4509; SW = 2424
PW1 = "CROP_GLOW44";  PW1_SC = 2724; PW1_SHOCK = "#854d0e"; PW1_ICON = "🌽💫"
PW2 = "FIELD_GLOW44"; PW2_SC = 2726; PW2_SHOCK = "#713f12"; PW2_ICON = "🌾✨"

# Prev-batch anchors (batch 814)
PREV_FOX_FULL = "topazolite_fox9e"
PREV_FOX_COLOR = "#a3e635"; PREV_FOX_GLOW = "#ecfccb"
PREV_PW1  = "SPORE_GLOW44"; PREV_PW1_ICON = "🍄💫"
PREV_DRAW_FOX = "drawTopazoliteFox9e"
PREV_FOX_R = "BASE_R*2.59"

# ── Step 1 – 8 achievements ───────────────────────────────────────────────────
ACH = f'''  {{ id:"{FOX_TYPE}_tap",          label:"Tschermigite Fox Tap",        desc:"Tap Tschermigite Fox target",           icon:"{FOX_PEAK}", xp:62 }},
  {{ id:"{FOX_TYPE}_peak",         label:"Tschermigite Fox Peak",       desc:"Reach peak with Tschermigite Fox",      icon:"{FOX_PEAK}", xp:124 }},
  {{ id:"{ORB_TYPE}_tap",          label:"Tsumcorite Orb Tap",          desc:"Tap Tsumcorite Orb target",             icon:"{ORB_PEAK}", xp:62 }},
  {{ id:"{ORB_TYPE}_peak",         label:"Tsumcorite Orb Peak",         desc:"Reach peak with Tsumcorite Orb",        icon:"{ORB_PEAK}", xp:124 }},
  {{ id:"{PW1.lower()}_use",       label:"Crop Glow",                   desc:"Trigger CROP_GLOW44 power-up",          icon:"🌽", xp:62 }},
  {{ id:"{PW1.lower()}_max",       label:"Crop Glow Max",               desc:"Trigger CROP_GLOW44 at max streak",     icon:"🌽", xp:124 }},
  {{ id:"{PW2.lower()}_use",       label:"Field Glow",                  desc:"Trigger FIELD_GLOW44 power-up",         icon:"🌾", xp:62 }},
  {{ id:"{PW2.lower()}_max",       label:"Field Glow Max",              desc:"Trigger FIELD_GLOW44 at max streak",    icon:"🌾", xp:124 }},
...
CMT = f'''// {PW1} — +{PW1_SC} crop glow bonus
    // {PW2} — +{PW2_SC} field glow bonus
    // {PREV_PW1}'''
...
DRAWS = f'''function drawTschermigiteFox9e(ctx,r,ts,sp){{
  const p=ts/sp,pu=p<0.5?p*2:2-p*2;
  ctx.save();
  ctx.shadowColor="{FOX_GLOW}";ctx.shadowBlur=20+pu*18;
  ctx.beginPath();ctx.arc(0,0,r,0,Math.PI*2);
  const g=ctx.createRadialGradient(0,0,r*0.1,0,0,r);
  g.addColorStop(0,"#ffffff");g.addColorStop(0.5,"{FOX_COLOR}");g.addColorStop(1,"#94a3b8");
...
function drawTsumcoriteOrb9e(ctx,r,ts,tp){{
  const p=ts/tp,pu=p<0.5?p*2:2-p*2;
  ctx.save();
  ctx.shadowColor="{ORB_GLOW}";ctx.shadowBlur=18+pu*16;
  ctx.beginPath();ctx.arc(0,0,r,0,Math.PI*2);
  const g=ctx.createRadialGradient(0,0,r*0.1,0,0,r);
  g.addColorStop(0,"#fca5a5");g.addColorStop(0.5,"{ORB_COLOR}");g.addColorStop(1,"#7f1d1d");
  ctx.fillStyle=g;ctx.fill();
...
DISP = f'''  else if(t.type==="{FOX_TYPE}"){{drawTschermigiteFox9e(ctx,t.radius,ts-t.born,t.lifetime);}}
  else if(t.type==="{ORB_TYPE}"){{drawTsumcoriteOrb9e(ctx,t.radius,ts-t.born,t.lifetime);}}
...
with open(SRC, "w") as f:
    f.write(src)
print(f"Batch 815 done! +{len(src)-original_len} bytes")
