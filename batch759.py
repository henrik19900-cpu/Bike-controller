...
OSC   = "0.4285"
SW    = "2312"
FOX_C = "#065f46"; FOX_G = "#ecfdf5"
ORB_C = "#92400e"; ORB_G = "#fffbeb"
FOX_L = "#6ee7b7"; FOX_M = "#059669"; FOX_D = "#022c22"
FOX_RL,FOX_RG,FOX_RB = 110,231,183
ORB_L = "#fde68a"; ORB_M = "#d97706"; ORB_D = "#451a03"
ORB_RL,ORB_RG,ORB_RB = 253,230,138
PW1C = "#34d399"; PW2C = "#fbbf24"
PEAK_FOX = "🌳"; PEAK_ORB = "🦁"

# STEP 1
A1_ANCHOR = '{ id:"fahlerz_fox9e_tap", label:"Fahlerz Fox Tap"'
assert A1_ANCHOR in code, "S1 anchor missing"
A1_NEW = (
  '{ id:"fenaksite_fox9e_tap", label:"Fenaksite Fox Tap", desc:"Tap Fenaksite Fox", icon:"🦊", xp:62 },\n'
  '  { id:"fenaksite_fox9e_peak", label:"Fenaksite Fox Peak", desc:"Hit Fenaksite Fox at peak", icon:"🌳", xp:78 },\n'
  '  { id:"fluellite_orb9e_tap", label:"Fluellite Orb Tap", desc:"Tap Fluellite Orb", icon:"🔮", xp:60 },\n'
  '  { id:"fluellite_orb9e_peak", label:"Fluellite Orb Peak", desc:"Hit Fluellite Orb at peak", icon:"🦁", xp:76 },\n'
  '  { id:"jungle_glow44_use", label:"Jungle Glow Use", desc:"Collect Jungle Glow power-up", icon:"🌴", xp:44 },\n'
  '  { id:"jungle_glow44_max", label:"Jungle Glow Max", desc:"Collect 5 Jungle Glow power-ups", icon:"🌴", xp:88 },\n'
  '  { id:"savanna_glow44_use", label:"Savanna Glow Use", desc:"Collect Savanna Glow power-up", icon:"🦁", xp:44 },\n'
  '  { id:"savanna_glow44_max", label:"Savanna Glow Max", desc:"Collect 5 Savanna Glow power-ups", icon:"🦁", xp:88 },\n'
  '  { id:"fahlerz_fox9e_tap", label:"Fahlerz Fox Tap"'
)
code = code.replace(A1_ANCHOR, A1_NEW, 1)
assert '{ id:"fenaksite_fox9e_tap"' in code, "S1 failed"
print("S1 ok")

# STEP 2
A2_ANCHOR = '"BOREAL_GLOW44","TAIGA_GLOW44","TUNDRA_GLOW44"'
assert A2_ANCHOR in code, "S2 anchor missing"
A2_NEW = f'"{PW1}","{PW2}","BOREAL_GLOW44","TAIGA_GLOW44","TUNDRA_GLOW44"'
code = code.replace(A2_ANCHOR, A2_NEW, 1)
assert f'"{PW1}"' in code, "S2 failed"
print("S2 ok")

# STEP 3
A3_ANCHOR = '} else if(ptype==="BOREAL_GLOW44"){'
...
# STEP 4
A4_ANCHOR = '// BOREAL_GLOW44 — +2496 boreal glow bonus'
assert A4_ANCHOR in code, "S4 anchor missing"
A4_NEW = (
  f'// {PW1} — +{PW1SC} jungle glow bonus\n'
  f'      // {PW2} — +{PW2SC} savanna glow bonus\n'
  f'      // BOREAL_GLOW44 — +2496 boreal glow bonus'
)
...
# STEP 5
A5_ANCHOR = 'function drawFahlerzFox9e('
assert A5_ANCHOR in code, "S5 anchor missing"
DRAW = (
f'function drawFenaksiteFox9e(ctx,r,ts,sp){{\n'
...
f'  ctx.restore();\n'
f'}}\n'
f'function drawFluelliteOrb9e(ctx,r,ts,tp){{\n'
...
f'  ctx.restore();\n'
f'}}\n'
f'function drawFahlerzFox9e('
)
code = code.replace(A5_ANCHOR, DRAW, 1)
assert 'function drawFenaksiteFox9e(' in code, "S5 failed"
print("S5 ok")

# STEP 6
A6_ANCHOR = '  else if(t.type==="fahlerz_fox9e"){'
...
f'    drawFenaksiteFox9e(ctx,t.radius,ts,sp2);ctx.restore();\n'
f'  }}\n'
f'  else if(t.type==="{ORB}"){{\n'
f'    ctx.save();ctx.translate(t.x,t.y);\n'
f'    const tp2=Math.min(1,(ts-t.born)/{SW});\n'
f'    drawFluelliteOrb9e(ctx,t.radius,ts,tp2);ctx.restore();\n'
f'  }}\n'
f'  else if(t.type==="fahlerz_fox9e"){{'
)
...
# STEP 7
A7_ANCHOR = 'if(hit.type==="fahlerz_fox9e"){'
...
f'      if(isPeak)unlock("fenaksite_fox9e_peak");\n'
f'      unlock("fenaksite_fox9e_tap");\n'
...
f'      addParticle(hit.x,hit.y,"popup",isPeak?"PEAK! +"+pts:"🔮 +"+pts);\n'
f'      if(isPeak)unlock("fluellite_orb9e_peak");\n'
f'      unlock("fluellite_orb9e_tap");\n'
f'      updateMissions({{...gs.sessionStats,rareHits:gs.sessionStats.rareHits}});\n'
f'      sfx("epic");vibrate([40,10,20]);\n'
f'    }}else if(hit.type==="fahlerz_fox9e"){{'
)
...
# STEP 8
A8_OLD = ('      type="fahlerz_fox9e";color="#1e40af";glow="#eff6ff";\n'
          '    } else if('+COND100F+'){\n'
          '      type="faujasite_orb9e"')
...
A8_NEW = (
    ...
    f'    }} else if({COND100F}){{\n'
    f'      type="fahlerz_fox9e";color="#1e40af";glow="#eff6ff";\n'
    f'    }} else if({COND100F}){{\n'
    f'      type="faujasite_orb9e"'
)
...
# STEP 9
A9_ANCHOR = 'type==="fahlerz_fox9e"?BASE_R*2.03:type==="faujasite_orb9e"?BASE_R*2.02:'
...
# STEP 10
A10_ANCHOR = 'BOREAL_GLOW44:"🌲✨",TAIGA_GLOW44:"🌿✨",TUNDRA_GLOW44:"🌨️✨"'
cnt = code.count(A10_ANCHOR)
assert cnt == 2, f"S10 anchor count={cnt}, expected 2"
A10_NEW = f'{PW1}:"🌴✨",{PW2}:"🦁✨",BOREAL_GLOW44:"🌲✨",TAIGA_GLOW44:"🌿✨",TUNDRA_GLOW44:"🌨️✨"'
...
with open(SRC, "w") as f:
    f.write(code)
print(f"Batch 759 done! +{len(code)-orig_len} bytes")
