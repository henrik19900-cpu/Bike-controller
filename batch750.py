...
OSC   = "0.4249"
SW    = "2294"
FOX_C = "#0c4a6e"; FOX_G = "#f0f9ff"
ORB_C = "#92400e"; ORB_G = "#fffbeb"
FOX_L = "#bae6fd"; FOX_M = "#0284c7"; FOX_D = "#0c4a6e"
FOX_RL,FOX_RG,FOX_RB = 186,230,253
ORB_L = "#fcd34d"; ORB_M = "#b45309"; ORB_D = "#451a03"
ORB_RL,ORB_RG,ORB_RB = 252,211,77
PW1C = "#2563eb"; PW2C = "#10b981"
PEAK_FOX = "❄️"; PEAK_ORB = "🌻"

# STEP 1
A1_ANCHOR = '{ id:"connellite_fox9e_tap", label:"Connellite Fox Tap"'
assert A1_ANCHOR in code, "S1 anchor missing"
A1_NEW = (
  '{ id:"cryolite_fox9e_tap", label:"Cryolite Fox Tap", desc:"Tap Cryolite Fox", icon:"🦊", xp:62 },\n'
  '  { id:"cryolite_fox9e_peak", label:"Cryolite Fox Peak", desc:"Hit Cryolite Fox at peak", icon:"❄️", xp:78 },\n'
  '  { id:"cryptomelane_orb9e_tap", label:"Cryptomelane Orb Tap", desc:"Tap Cryptomelane Orb", icon:"🔮", xp:60 },\n'
  '  { id:"cryptomelane_orb9e_peak", label:"Cryptomelane Orb Peak", desc:"Hit Cryptomelane Orb at peak", icon:"🌻", xp:76 },\n'
  '  { id:"wave_glow44_use", label:"Wave Glow Use", desc:"Collect Wave Glow power-up", icon:"🌊", xp:44 },\n'
  '  { id:"wave_glow44_max", label:"Wave Glow Max", desc:"Collect 5 Wave Glow power-ups", icon:"🌊", xp:88 },\n'
  '  { id:"gale_glow44_use", label:"Gale Glow Use", desc:"Collect Gale Glow power-up", icon:"💨", xp:44 },\n'
  '  { id:"gale_glow44_max", label:"Gale Glow Max", desc:"Collect 5 Gale Glow power-ups", icon:"💨", xp:88 },\n'
  '  { id:"connellite_fox9e_tap", label:"Connellite Fox Tap"'
)
code = code.replace(A1_ANCHOR, A1_NEW, 1)
assert '{ id:"cryolite_fox9e_tap"' in code, "S1 failed"
print("S1 ok")

# STEP 2
A2_ANCHOR = '"CREST_GLOW44","TIDE_GLOW44","VORTEX_GLOW44"'
assert A2_ANCHOR in code, "S2 anchor missing"
A2_NEW = f'"{PW1}","{PW2}","CREST_GLOW44","TIDE_GLOW44","VORTEX_GLOW44"'
code = code.replace(A2_ANCHOR, A2_NEW, 1)
assert f'"{PW1}"' in code, "S2 failed"
print("S2 ok")

# STEP 3
A3_ANCHOR = '} else if(ptype==="CREST_GLOW44"){'
...
# STEP 4
A4_ANCHOR = '// CREST_GLOW44 — +2460 crest glow bonus'
assert A4_ANCHOR in code, "S4 anchor missing"
A4_NEW = (
  f'// {PW1} — +{PW1SC} wave glow bonus\n'
  f'      // {PW2} — +{PW2SC} gale glow bonus\n'
  f'      // CREST_GLOW44 — +2460 crest glow bonus'
)
...
# STEP 5
A5_ANCHOR = 'function drawConnelliteFox9e('
assert A5_ANCHOR in code, "S5 anchor missing"
DRAW = (
f'function drawCryoliteFox9e(ctx,r,ts,sp){{\n'
...
f'  ctx.restore();\n'
f'}}\n'
f'function drawCryptomolaneOrb9e(ctx,r,ts,tp){{\n'
...
f'  ctx.restore();\n'
f'}}\n'
f'function drawConnelliteFox9e('
)
code = code.replace(A5_ANCHOR, DRAW, 1)
assert 'function drawCryoliteFox9e(' in code, "S5 failed"
print("S5 ok")

# STEP 6
A6_ANCHOR = '  else if(t.type==="connellite_fox9e"){'
...
f'    drawCryoliteFox9e(ctx,t.radius,ts,sp2);ctx.restore();\n'
f'  }}\n'
f'  else if(t.type==="{ORB}"){{\n'
f'    ctx.save();ctx.translate(t.x,t.y);\n'
f'    const tp2=Math.min(1,(ts-t.born)/{SW});\n'
f'    drawCryptomolaneOrb9e(ctx,t.radius,ts,tp2);ctx.restore();\n'
f'  }}\n'
f'  else if(t.type==="connellite_fox9e"){{'
)
...
# STEP 7
A7_ANCHOR = 'if(hit.type==="connellite_fox9e"){'
...
f'      if(isPeak)unlock("cryolite_fox9e_peak");\n'
f'      unlock("cryolite_fox9e_tap");\n'
...
f'      addParticle(hit.x,hit.y,"popup",isPeak?"PEAK! +"+pts:"🔮 +"+pts);\n'
f'      if(isPeak)unlock("cryptomelane_orb9e_peak");\n'
f'      unlock("cryptomelane_orb9e_tap");\n'
f'      updateMissions({{...gs.sessionStats,rareHits:gs.sessionStats.rareHits}});\n'
f'      sfx("epic");vibrate([40,10,20]);\n'
f'    }}else if(hit.type==="connellite_fox9e"){{'
)
...
# STEP 8
A8_OLD = ('      type="connellite_fox9e";color="#0e7490";glow="#ecfeff";\n'
          '    } else if('+COND100F+'){\n'
          '      type="creedite_orb9e"')
...
A8_NEW = (
    ...
    f'    }} else if({COND100F}){{\n'
    f'      type="connellite_fox9e";color="#0e7490";glow="#ecfeff";\n'
    f'    }} else if({COND100F}){{\n'
    f'      type="creedite_orb9e"'
)
...
# STEP 9
A9_ANCHOR = 'type==="connellite_fox9e"?BASE_R*1.94:type==="creedite_orb9e"?BASE_R*1.93:'
...
# STEP 10
A10_ANCHOR = 'CREST_GLOW44:"🌊✨",TIDE_GLOW44:"🌊✨",VORTEX_GLOW44:"🌀✨"'
cnt = code.count(A10_ANCHOR)
assert cnt == 2, f"S10 anchor count={cnt}, expected 2"
A10_NEW = f'{PW1}:"🌊✨",{PW2}:"💨✨",CREST_GLOW44:"🌊✨",TIDE_GLOW44:"🌊✨",VORTEX_GLOW44:"🌀✨"'
...
with open(SRC, "w") as f:
    f.write(code)
print(f"Batch 750 done! +{len(code)-orig_len} bytes")
