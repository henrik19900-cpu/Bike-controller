...
OSC   = "0.4281"
SW    = "2310"
FOX_C = "#1e40af"; FOX_G = "#eff6ff"
ORB_C = "#166534"; ORB_G = "#f0fdf4"
FOX_L = "#bfdbfe"; FOX_M = "#3b82f6"; FOX_D = "#1e1b4b"
FOX_RL,FOX_RG,FOX_RB = 191,219,254
ORB_L = "#bbf7d0"; ORB_M = "#22c55e"; ORB_D = "#052e16"
ORB_RL,ORB_RG,ORB_RB = 187,247,208
PW1C = "#86efac"; PW2C = "#4ade80"
PEAK_FOX = "🌀"; PEAK_ORB = "🌲"

# STEP 1
A1_ANCHOR = '{ id:"enargite_fox9e_tap", label:"Enargite Fox Tap"'
assert A1_ANCHOR in code, "S1 anchor missing"
A1_NEW = (
  '{ id:"fahlerz_fox9e_tap", label:"Fahlerz Fox Tap", desc:"Tap Fahlerz Fox", icon:"🦊", xp:62 },\n'
  '  { id:"fahlerz_fox9e_peak", label:"Fahlerz Fox Peak", desc:"Hit Fahlerz Fox at peak", icon:"🌀", xp:78 },\n'
  '  { id:"faujasite_orb9e_tap", label:"Faujasite Orb Tap", desc:"Tap Faujasite Orb", icon:"🔮", xp:60 },\n'
  '  { id:"faujasite_orb9e_peak", label:"Faujasite Orb Peak", desc:"Hit Faujasite Orb at peak", icon:"🌲", xp:76 },\n'
  '  { id:"boreal_glow44_use", label:"Boreal Glow Use", desc:"Collect Boreal Glow power-up", icon:"🌲", xp:44 },\n'
  '  { id:"boreal_glow44_max", label:"Boreal Glow Max", desc:"Collect 5 Boreal Glow power-ups", icon:"🌲", xp:88 },\n'
  '  { id:"taiga_glow44_use", label:"Taiga Glow Use", desc:"Collect Taiga Glow power-up", icon:"🌿", xp:44 },\n'
  '  { id:"taiga_glow44_max", label:"Taiga Glow Max", desc:"Collect 5 Taiga Glow power-ups", icon:"🌿", xp:88 },\n'
  '  { id:"enargite_fox9e_tap", label:"Enargite Fox Tap"'
)
code = code.replace(A1_ANCHOR, A1_NEW, 1)
assert '{ id:"fahlerz_fox9e_tap"' in code, "S1 failed"
print("S1 ok")

# STEP 2
A2_ANCHOR = '"TUNDRA_GLOW44","POLAR_GLOW44","GLACIER_GLOW44"'
assert A2_ANCHOR in code, "S2 anchor missing"
A2_NEW = f'"{PW1}","{PW2}","TUNDRA_GLOW44","POLAR_GLOW44","GLACIER_GLOW44"'
code = code.replace(A2_ANCHOR, A2_NEW, 1)
assert f'"{PW1}"' in code, "S2 failed"
print("S2 ok")

# STEP 3
A3_ANCHOR = '} else if(ptype==="TUNDRA_GLOW44"){'
...
# STEP 4
A4_ANCHOR = '// TUNDRA_GLOW44 — +2492 tundra glow bonus'
assert A4_ANCHOR in code, "S4 anchor missing"
A4_NEW = (
  f'// {PW1} — +{PW1SC} boreal glow bonus\n'
  f'      // {PW2} — +{PW2SC} taiga glow bonus\n'
  f'      // TUNDRA_GLOW44 — +2492 tundra glow bonus'
)
...
# STEP 5
A5_ANCHOR = 'function drawEnargiteFox9e('
assert A5_ANCHOR in code, "S5 anchor missing"
DRAW = (
f'function drawFahlerzFox9e(ctx,r,ts,sp){{\n'
...
f'  ctx.restore();\n'
f'}}\n'
f'function drawFaujasiteOrb9e(ctx,r,ts,tp){{\n'
...
f'  ctx.restore();\n'
f'}}\n'
f'function drawEnargiteFox9e('
)
code = code.replace(A5_ANCHOR, DRAW, 1)
assert 'function drawFahlerzFox9e(' in code, "S5 failed"
print("S5 ok")

# STEP 6
A6_ANCHOR = '  else if(t.type==="enargite_fox9e"){'
...
f'    drawFahlerzFox9e(ctx,t.radius,ts,sp2);ctx.restore();\n'
f'  }}\n'
f'  else if(t.type==="{ORB}"){{\n'
f'    ctx.save();ctx.translate(t.x,t.y);\n'
f'    const tp2=Math.min(1,(ts-t.born)/{SW});\n'
f'    drawFaujasiteOrb9e(ctx,t.radius,ts,tp2);ctx.restore();\n'
f'  }}\n'
f'  else if(t.type==="enargite_fox9e"){{'
)
...
# STEP 7
A7_ANCHOR = 'if(hit.type==="enargite_fox9e"){'
...
f'      if(isPeak)unlock("fahlerz_fox9e_peak");\n'
f'      unlock("fahlerz_fox9e_tap");\n'
...
f'      addParticle(hit.x,hit.y,"popup",isPeak?"PEAK! +"+pts:"🔮 +"+pts);\n'
f'      if(isPeak)unlock("faujasite_orb9e_peak");\n'
f'      unlock("faujasite_orb9e_tap");\n'
f'      updateMissions({{...gs.sessionStats,rareHits:gs.sessionStats.rareHits}});\n'
f'      sfx("epic");vibrate([40,10,20]);\n'
f'    }}else if(hit.type==="enargite_fox9e"){{'
)
...
# STEP 8
A8_OLD = ('      type="enargite_fox9e";color="#9a3412";glow="#fff7ed";\n'
          '    } else if('+COND100F+'){\n'
          '      type="eosphorite_orb9e"')
...
A8_NEW = (
    ...
    f'    }} else if({COND100F}){{\n'
    f'      type="enargite_fox9e";color="#9a3412";glow="#fff7ed";\n'
    f'    }} else if({COND100F}){{\n'
    f'      type="eosphorite_orb9e"'
)
...
# STEP 9
A9_ANCHOR = 'type==="enargite_fox9e"?BASE_R*2.02:type==="eosphorite_orb9e"?BASE_R*2.01:'
...
# STEP 10
A10_ANCHOR = 'TUNDRA_GLOW44:"🌨️✨",POLAR_GLOW44:"🧊✨",GLACIER_GLOW44:"🧊✨"'
cnt = code.count(A10_ANCHOR)
assert cnt == 2, f"S10 anchor count={cnt}, expected 2"
A10_NEW = f'{PW1}:"🌲✨",{PW2}:"🌿✨",TUNDRA_GLOW44:"🌨️✨",POLAR_GLOW44:"🧊✨",GLACIER_GLOW44:"🧊✨"'
...
with open(SRC, "w") as f:
    f.write(code)
print(f"Batch 758 done! +{len(code)-orig_len} bytes")
