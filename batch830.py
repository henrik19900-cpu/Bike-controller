...
ORB_COLOR = "#7c2d12"; ORB_GLOW = "#ffedd5"; ORB_PEAK = "🔶"
FOX_SC   = 644;  ORB_SC  = 639
FOX_R    = "BASE_R*2.75"; ORB_R = "BASE_R*2.74"
OSC      = 0.4569; SW = 2454
PW1 = "MYSTIC2_GLOW44"; PW1_SC = 2784; PW1_SHOCK = "#7c3aed"; PW1_ICON = "🔮💫"
PW2 = "PRIMAL2_GLOW44"; PW2_SC = 2786; PW2_SHOCK = "#6d28d9"; PW2_ICON = "⚔️✨"

# Prev-batch anchors (batch 829)
PREV_FOX_FULL = "austinite_fox9e"
PREV_FOX_COLOR = "#6d28d9"; PREV_FOX_GLOW = "#ede9fe"
PREV_PW1  = "LUSTER2_GLOW44"; PREV_PW1_ICON = "✨💫"
PREV_DRAW_FOX = "drawAustiniteFox9e"
PREV_FOX_R = "BASE_R*2.74"

# ── Step 1 – 8 achievements ───────────────────────────────────────────────────
ACH = f'''  {{ id:"{FOX_TYPE}_tap",          label:"Balangeroite Fox Tap",        desc:"Tap Balangeroite Fox target",           icon:"{FOX_PEAK}", xp:62 }},
  {{ id:"{FOX_TYPE}_peak",         label:"Balangeroite Fox Peak",       desc:"Reach peak with Balangeroite Fox",      icon:"{FOX_PEAK}", xp:124 }},
  {{ id:"{ORB_TYPE}_tap",          label:"Bannermanite Orb Tap",        desc:"Tap Bannermanite Orb target",           icon:"{ORB_PEAK}", xp:62 }},
  {{ id:"{ORB_TYPE}_peak",         label:"Bannermanite Orb Peak",       desc:"Reach peak with Bannermanite Orb",      icon:"{ORB_PEAK}", xp:124 }},
  {{ id:"{PW1.lower()}_use",       label:"Mystic2 Glow",                desc:"Trigger MYSTIC2_GLOW44 power-up",       icon:"🔮", xp:62 }},
  {{ id:"{PW1.lower()}_max",       label:"Mystic2 Glow Max",            desc:"Trigger MYSTIC2_GLOW44 at max streak",  icon:"🔮", xp:124 }},
  {{ id:"{PW2.lower()}_use",       label:"Primal2 Glow",                desc:"Trigger PRIMAL2_GLOW44 power-up",       icon:"⚔️", xp:62 }},
  {{ id:"{PW2.lower()}_max",       label:"Primal2 Glow Max",            desc:"Trigger PRIMAL2_GLOW44 at max streak",  icon:"⚔️", xp:124 }},
...
CMT = f'''// {PW1} — +{PW1_SC} mystic2 glow bonus
    // {PW2} — +{PW2_SC} primal2 glow bonus
    // {PREV_PW1}'''
...
DRAWS = f'''function drawBalangeroiteFox9e(ctx,r,ts,sp){{
...
function drawBannermaniteOrb9e(ctx,r,ts,tp){{
...
DISP = f'''  else if(t.type==="{FOX_TYPE}"){{drawBalangeroiteFox9e(ctx,t.radius,ts-t.born,t.lifetime);}}
  else if(t.type==="{ORB_TYPE}"){{drawBannermaniteOrb9e(ctx,t.radius,ts-t.born,t.lifetime);}}
...
with open(SRC, "w") as f:
    f.write(src)
print(f"Batch 830 done! +{len(src)-original_len} bytes")
