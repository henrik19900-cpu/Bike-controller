...
FOX_SC   = 604;  ORB_SC  = 599
FOX_R    = "BASE_R*2.55"; ORB_R = "BASE_R*2.54"
OSC      = 0.4489; SW = 2414
PW1 = "BARK_GLOW44";  PW1_SC = 2704; PW1_SHOCK = "#92400e"; PW1_ICON = "🪵💫"
PW2 = "BOUGH_GLOW44"; PW2_SC = 2706; PW2_SHOCK = "#78350f"; PW2_ICON = "🌳✨"

# Prev-batch anchors (batch 809)
PREV_FOX_FULL = "stilpnomelane_fox9e"
PREV_ORB_FULL = "stromeyerite_orb9e"
PREV_FOX_COLOR = "#064e3b"; PREV_FOX_GLOW = "#d1fae5"
PREV_PW1  = "SEED_GLOW44"; PREV_PW1_ICON = "🌱💫"
PREV_DRAW_FOX = "drawStilpnomelaneFox9e"
PREV_FOX_R = "BASE_R*2.54"

# ── Step 1 – 8 achievements ───────────────────────────────────────────────────
ACH = f'''  {{ id:"{FOX_TYPE}_tap",          label:"Strunzite Fox Tap",           desc:"Tap Strunzite Fox target",              icon:"{FOX_PEAK}", xp:62 }},
  {{ id:"{FOX_TYPE}_peak",         label:"Strunzite Fox Peak",          desc:"Reach peak with Strunzite Fox",         icon:"{FOX_PEAK}", xp:124 }},
  {{ id:"{ORB_TYPE}_tap",          label:"Titanite Orb Tap",            desc:"Tap Titanite Orb target",               icon:"{ORB_PEAK}", xp:62 }},
  {{ id:"{ORB_TYPE}_peak",         label:"Titanite Orb Peak",           desc:"Reach peak with Titanite Orb",          icon:"{ORB_PEAK}", xp:124 }},
  {{ id:"{PW1.lower()}_use",       label:"Bark Glow",                   desc:"Trigger BARK_GLOW44 power-up",          icon:"🪵", xp:62 }},
  {{ id:"{PW1.lower()}_max",       label:"Bark Glow Max",               desc:"Trigger BARK_GLOW44 at max streak",     icon:"🪵", xp:124 }},
  {{ id:"{PW2.lower()}_use",       label:"Bough Glow",                  desc:"Trigger BOUGH_GLOW44 power-up",         icon:"🌳", xp:62 }},
  {{ id:"{PW2.lower()}_max",       label:"Bough Glow Max",              desc:"Trigger BOUGH_GLOW44 at max streak",    icon:"🌳", xp:124 }},
...
CMT = f'''// {PW1} — +{PW1_SC} bark glow bonus
    // {PW2} — +{PW2_SC} bough glow bonus
    // {PREV_PW1}'''
...
DRAWS = f'''function drawStrunziteFox9e(ctx,r,ts,sp){{
  const p=ts/sp,pu=p<0.5?p*2:2-p*2;
  ctx.save();
  ctx.shadowColor="{FOX_GLOW}";ctx.shadowBlur=20+pu*18;
  ctx.beginPath();ctx.arc(0,0,r,0,Math.PI*2);
  const g=ctx.createRadialGradient(0,0,r*0.1,0,0,r);
  g.addColorStop(0,"#fcd34d");g.addColorStop(0.5,"{FOX_COLOR}");g.addColorStop(1,"#78350f");
...
function drawTitaniteOrb9e(ctx,r,ts,tp){{
...
DISP = f'''  else if(t.type==="{FOX_TYPE}"){{drawStrunziteFox9e(ctx,t.radius,ts-t.born,t.lifetime);}}
  else if(t.type==="{ORB_TYPE}"){{drawTitaniteOrb9e(ctx,t.radius,ts-t.born,t.lifetime);}}
...
with open(SRC, "w") as f:
    f.write(src)
print(f"Batch 810 done! +{len(src)-original_len} bytes")
